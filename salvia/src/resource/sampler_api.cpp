...
void tex2Dgrad_ps(vec4 *result, uint32_t mask, sampler *samp, vec2 *coord, vec2 const *ddx,
                  vec2 const *ddy) {
  if (mask) {
...
void tex2Dbias_ps(vec4 *results, uint32_t mask, sampler *samp, vec4 *coords, vec2 const *ddxs,
                  vec2 const *ddys) {
