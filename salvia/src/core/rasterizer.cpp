...
bool is_valid_quad(size_t quad_x, size_t quad_y) {
  size_t debug_start_x = 200;
  size_t debug_start_y = 220;
  size_t width = 4;
  size_t height = 4;
...
void rasterizer::update(render_state const* state) {
  ...
  pipeline_stat_ =
      state->asyncs[static_cast<uint32_t>(async_object_ids::pipeline_statistics)].get();
  internal_stat_ =
      state->asyncs[static_cast<uint32_t>(async_object_ids::internal_statistics)].get();
  pipeline_prof_ = state->asyncs[static_cast<uint32_t>(async_object_ids::pipeline_profiles)].get();

  if (pipeline_stat_) {
    acc_ia_primitives_ =
        &async_pipeline_statistics::accumulate<pipeline_statistic_id::ia_primitives>;
    ...
    acc_ps_invocations_ =
        &async_pipeline_statistics::accumulate<pipeline_statistic_id::ps_invocations>;
  } else {
  ...
  if (internal_stat_) {
    acc_backend_input_pixels_ =
        &async_internal_statistics::accumulate<internal_statistics_id::backend_input_pixels>;
  } else {
...
void rasterizer::subdivide_tile(int left,
  ...
  for (int iy = 0; iy < 4; ++iy) {
    ...
    {
      __m128 mstepx = _mm_mul_ps(_mm_shuffle_ps(mstepx3, mstepx3, _MM_SHUFFLE(1, 1, 1, 1)),
                                 _mm_set_ps(3, 2, 1, 0));
    ...
    {
      __m128 mstepx = _mm_mul_ps(_mm_shuffle_ps(mstepx3, mstepx3, _MM_SHUFFLE(2, 2, 2, 2)),
                                 _mm_set_ps(3, 2, 1, 0));
    ...
    mask_rej = _mm_or_ps(
        mask_rej, _mm_cmpge_ps(_mm_set1_ps(y_min), _mm_cvtepi32_ps(_mm_add_epi32(miy, minewh))));
  ...
  for (int ty = 0; ty < 4; ++ty) {
    ...
    for (int tx = 0; tx < 4; ++tx) {
      ...
      if ((x_min < x + new_w) && (x_max >= x) && (y_min < y + new_h) && (y_max >= y)) {
        int rejection = 0;
        int acception = 1;
  ...
  while (test_region_size[src_stage] > 0) {
    ...
    for (size_t ivp = 0; ivp < test_region_size[src_stage]; ++ivp) {
      ...
      case TVT_FULL: {
        ...
        const int vpright =
            min(vpleft0 + cur_region.x + cur_region.w * 4, static_cast<uint32_t>(target_vp_right));
        const int vpbottom =
            min(vptop0 + cur_region.y + cur_region.h * 4, static_cast<uint32_t>(target_vp_bottom));
...
void rasterizer::threaded_dispatch_primitive(thread_context const* thread_ctx) {
  ...
  while (current_package.valid()) {
    ...
    for (size_t i = prim_range.first; i < prim_range.second; ++i) {
      ...
      const int sx =
          std::min(fast_floori(std::max(0.0f, x_min) / TILE_SIZE), static_cast<int>(tile_x_count_));
      const int sy =
          std::min(fast_floori(std::max(0.0f, y_min) / TILE_SIZE), static_cast<int>(tile_y_count_));
      const int ex = std::min(fast_ceili(std::max(0.0f, x_max) / TILE_SIZE) + 1,
                              static_cast<int>(tile_x_count_));
      const int ey = std::min(fast_ceili(std::max(0.0f, y_max) / TILE_SIZE) + 1,
                              static_cast<int>(tile_y_count_));
...
void rasterizer::compute_triangle_info(uint32_t i) {
  ...
  double dist[3] = {fabs(vert_pos[0]->x()) + fabs(vert_pos[0]->y()),
...
void rasterizer::draw_quad(uint32_t left,
  ...
  if (frame_buffer_->early_z_enabled()) {
    tested_quad_mask =
        frame_buffer_->early_z_test_quad(left, top, quad_mask, depth, triangle_ctx->aa_z_offset);
  }
