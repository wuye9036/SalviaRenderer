...
vec4 get_vec4(format fmt, float default_wcomp, const void* data) {
  ...
  switch (fmt) {
  case format_r32_float: return vec4(floats[0], 0.0f, 0.0f, default_wcomp);
  case format_r32g32_float: return vec4(floats[0], floats[1], 0.0f, default_wcomp);
  case format_r32g32b32_float: return vec4(floats[0], floats[1], floats[2], default_wcomp);
  case format_r32g32b32a32_float: return vec4(floats[0], floats[1], floats[2], floats[3]);
  case format_r32g32b32a32_sint: return *reinterpret_cast<vec4 const*>(floats);
...
void stream_assembler::update_register_map(
    std::unordered_map<semantic_value, size_t> const& reg_map) {
...
void const* stream_assembler::element_address(input_element_desc const& elem_desc,
                                              size_t vert_index) const {
  auto buf_desc = stream_buffer_descs_ + elem_desc.input_slot;
  return buf_desc->buf->raw_data(elem_desc.aligned_byte_offset + buf_desc->stride * vert_index +
                                 buf_desc->offset);
}
