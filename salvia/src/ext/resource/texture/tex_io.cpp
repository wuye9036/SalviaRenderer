...
texture_ptr load_cube(renderer* rend, const vector<string>& filenames, pixel_format tex_format) {
  ...
  for (int i_cubeface = 0; i_cubeface < 6; ++i_cubeface) {
    std::unique_ptr<FIBITMAP, decltype(image_deleter)> cube_img(load_image(filenames[i_cubeface]),
                                                                image_deleter);
...
void save_surface(renderer* rend,
  ...
  case pixel_format_color_bgra8:
    ...
    fif = FIF_PNG;
    image =
        FreeImage_AllocateT(fit, surface_width, surface_height, 32, 0x0000FF, 0x00FF00, 0xFF0000);
    break;
