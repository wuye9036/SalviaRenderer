...
#include "salvia/ext/resource/mesh/collada.h"
...
struct skin_info {
  ...
  vector<pair<uint32_t /*skin_info.joint*/, uint32_t /*skin_info.weight*/>> vertex_skin_infos;
};
...
vector<mesh_ptr> build_mesh(dae_mesh_ptr m, skin_info *skinfo, renderer *render) {
  ...
  for (dae_triangles_ptr const &triangles : m->triangle_sets) {
    ...
    // Here we merge per-input index to an unified index, and re-order data to matching unified
    // index.
    ...
    for (size_t i_tri_vert = 0; i_tri_vert < triangles->count * 3; ++i_tri_vert) {
      ...
      for (size_t i_comp = 0; i_comp < vertex_attributes_count; ++i_comp) {
        index_group.indexes[i_comp] =
            (uint32_t)triangles->indexes[i_tri_vert * index_stride + offsets[i_comp]];
      }
      ...
      if (it != index_dict.end()) {
        ...
        if (skinfo) {
          size_t vert_index =
              (uint32_t)triangles->indexes[i_tri_vert * index_stride + vertex_index_offset];
          ...
          for (size_t i = 0; i < vert_skin_info_length; ++i) {
            ...
            vert_joint_weights.data_[i] =
                skinfo->weights[skinfo->vertex_skin_infos[skin_info_index].second];
          }
...
skin_info_ptr build_skin_info(dae_skin_ptr skin) {
  ...
  for (uint32_t i_vert = 0; i_vert < skin->weights->count; ++i_vert) {
    ...
    for (uint32_t i_joint = 0; i_joint < vcount; ++i_joint) {
      ...
      ret->vertex_skin_infos.push_back(
          make_pair(skin->weights->v[pos_of_joint_id], skin->weights->v[pos_of_weight]));
    }
...
void merge_buffer_to_mesh(mesh_impl *merged_mesh, /*OUT*/
  ...
  ieds.back().semantic_index =
      static_cast<uint32_t>(src_mesh_idx + src_mesh_count * ied.semantic_index);
...
mesh_ptr merge_mesh_for_morphing(mesh_ptr lm, mesh_ptr rm) {
  ...
  EFLIB_ASSERT_AND_IF(left_mesh->primcount_ == right_mesh->primcount_,
                      "Primitives amount is not matched.") {
    return mesh_ptr();
  }

  EFLIB_ASSERT_AND_IF(left_mesh->index_format_ == right_mesh->index_format_,
                      "Index format is not matched.") {
    return mesh_ptr();
  }
  ...
  ret->set_index_type(left_mesh->index_format_);
...
mesh_ptr create_morph_mesh_from_collada(salvia::core::renderer *render, std::string const &src,
  ...
  EFLIB_ASSERT_AND_IF(src_meshes.size() == dst_meshes.size(),
                      "Morph meshes need to be combined from two meshes with same size.") {
    return mesh_ptr();
  }
