...
salvia::resource::buffer_ptr mesh_impl::get_index_buffer() {
...
void mesh_impl::render() {
  ...
  if (device_->get_vertex_shader_code()) {
    cached_layout_ = device_->create_input_layout(
        &(elem_descs_[0]), elem_descs_.size(), device_->get_vertex_shader_code());
  } else if (device_->get_vertex_shader()) {
    cached_layout_ = device_->create_input_layout(
        &(elem_descs_[0]), elem_descs_.size(), device_->get_vertex_shader());
  } else {
    return;
  }
  if (!cached_layout_) {
  ...
  for (size_t i_buffer = 0; i_buffer < vertex_buffers_.size(); ++i_buffer) {
    device_->set_vertex_buffers(slots_[i_buffer],
                                1,
                                &vertex_buffers_[i_buffer],
                                &(strides_[i_buffer]),
                                &(offsets_[i_buffer]));
  }
...
mesh
*/
salvia::resource::buffer_ptr mesh_impl::create_buffer(size_t size) {
...
void mesh_impl::set_index_buffer(salvia::resource::buffer_ptr const& v) {
  index_buffer_ = v;
}
...
void mesh_impl::set_index_type(format fmt) {
  ...
  case format_r32_uint: index_format_ = fmt; break;
  default: EFLIB_ASSERT_UNEXPECTED();
  }
}

void mesh_impl::add_vertex_buffer(size_t slot,
                                  salvia::resource::buffer_ptr const& buf,
                                  size_t stride,
                                  size_t offset) {
  assert(buf);
...
salvia::ext::resource::attached_data_ptr mesh_impl::get_attached() {
  return attached_;
}
