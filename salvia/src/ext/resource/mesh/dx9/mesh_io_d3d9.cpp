...
mesh_ptr create_mesh_from_dx9mesh(salviar::renderer *psr, LPD3DXMESH dx_mesh) {
  ...
  for (size_t i_decl = 0; i_decl < MAX_FVF_DECL_SIZE; ++i_decl) {
    ...
    if (semantic_name) {
      elem_descs.push_back(input_element_desc(semantic_name, decl->UsageIndex,
                                              format_r32g32b32_float, 0, decl->Offset,
                                              input_per_vertex, 0));
    }
  }
...
mesh_ptr create_mesh_from_xfile(salviar::renderer *psr, d3d9_device *dev,
  ...
  D3DXLoadMeshFromX(filename.c_str(), D3DXMESH_SYSTEMMEM, d3ddev, &padjbuf, &pmaterials,
                    &peffectinstances, &material_counts, &pmesh);
