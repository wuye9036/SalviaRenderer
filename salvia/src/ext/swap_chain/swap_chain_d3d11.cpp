...
char const ps_data[] = "Texture2D sa_tex : register(t0); \n"
...
char const vs_data[] = "void VSMain(float2 pos : POSITION,\n"
...
public:
  ...
  void initialize() {
    ...
    if (mod_dxgi_ != nullptr) {
      DynamicCreateDXGIFactory1_ = reinterpret_cast<CreateDXGIFactory1Func>(
          ::GetProcAddress(mod_dxgi_, "CreateDXGIFactory1"));
    }

    if (mod_d3d11_ != nullptr) {
      DynamicD3D11CreateDeviceAndSwapChain_ = reinterpret_cast<D3D11CreateDeviceAndSwapChainFunc>(
  ...
  void present_impl() {
    if (!d3d_device_) {
      ...
      sc_desc.BufferDesc.Width = static_cast<UINT>(resolved_surface_->width());
      sc_desc.BufferDesc.Height = static_cast<UINT>(resolved_surface_->height());
...
#  endif
      ...
      // Setup the viewport
      D3D11_VIEWPORT vp;
      vp.Width = static_cast<float>(resolved_surface_->width());
      vp.Height = static_cast<float>(resolved_surface_->height());
      vp.MinDepth = 0.0f;
      vp.MaxDepth = 1.0f;
      vp.TopLeftX = 0;
      vp.TopLeftY = 0;
...
#  endif
      ...
      tex_desc.Width = static_cast<UINT>(resolved_surface_->width());
      tex_desc.Height = static_cast<UINT>(resolved_surface_->height());
