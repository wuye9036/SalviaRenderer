...
public:
  ...
  virtual texture_ptr
  create_tex2d(size_t width, size_t height, size_t num_samples, pixel_format fmt) = 0;
  virtual texture_ptr
  create_texcube(size_t width, size_t height, size_t num_samples, pixel_format fmt) = 0;
  ...
  // State set
  virtual result set_vertex_buffers(size_t starts_slot,
  ...
  virtual result
  set_vs_variable_value(std::string const& name, void const* pvariable, size_t sz) = 0;
  virtual result
  set_vs_variable_pointer(std::string const& name, void const* pvariable, size_t sz) = 0;
  ...
  virtual result set_render_targets(size_t color_target_count,
                                    surface_ptr const* color_targets,
                                    surface_ptr const& ds_target) = 0;
  ...
  get_data(async_object_ptr const& async_obj, void* data, bool do_not_wait) = 0;
  ...
  virtual result
  clear_depth_stencil(surface_ptr const& depth_stencil_target, uint32_t f, float d, uint32_t s) = 0;
...
shader::shader_object_ptr compile(std::string const& code,
