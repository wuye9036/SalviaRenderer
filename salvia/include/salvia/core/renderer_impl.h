...
public:
  ...
  result set_vertex_buffers(size_t starts_slot,
  ...
  result
  set_vs_variable_pointer(std::string const& name, void const* pvariable, size_t sz) override;
  ...
  result set_render_targets(size_t color_target_count,
  ...
  result clear_depth_stencil(surface_ptr const& depth_stencil_target,
                             uint32_t f,
                             float d,
                             uint32_t s) override;
  ...
  input_layout_ptr create_input_layout(input_element_desc const* elem_descs,
                                       size_t elems_count,
                                       shader::shader_object_ptr const& vs) override;
  input_layout_ptr create_input_layout(input_element_desc const* elem_descs,
                                       size_t elems_count,
                                       cpp_vertex_shader_ptr const& vs) override;
  buffer_ptr create_buffer(size_t size) override;
  texture_ptr
  create_tex2d(size_t width, size_t height, size_t num_samples, pixel_format fmt) override;
  texture_ptr
  create_texcube(size_t width, size_t height, size_t num_samples, pixel_format fmt) override;
