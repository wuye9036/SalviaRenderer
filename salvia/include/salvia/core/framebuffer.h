...
struct depth_stencil_op_desc {
  ...
  depth_stencil_op_desc()
      : stencil_fail_op(stencil_op_keep), stencil_depth_fail_op(stencil_op_keep),
        stencil_pass_op(stencil_op_keep), stencil_func(compare_function_always) {}
};
...
struct depth_stencil_desc {
  ...
  depth_stencil_desc()
      : depth_enable(true), depth_write_mask(true), depth_func(compare_function_less),
        stencil_enable(false), stencil_read_mask(0xFF), stencil_write_mask(0xFF) {}
};
...
public:
  ...
  uint32_t stencil_operation(bool front_face, bool depth_pass, bool stencil_pass, uint32_t ref,
...
private:
  ...
  void (*read_depth_stencil_)(float &depth, uint32_t &stencil, uint32_t stencil_mask,
                              void const *ds_data);
