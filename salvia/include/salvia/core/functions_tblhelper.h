...
#define FUNCTIONS_INITIALIZE(rettype, params, func_impl_name, count)                               \
  ...
  template <int id> struct FUNC_INITIALIZER(func_impl_name) {                                      \
  ...
  template <> struct FUNC_INITIALIZER(func_impl_name)<-1> {                                        \
    FUNC_INITIALIZER(func_impl_name)() {}                                                          \
  };                                                                                               \
  static FUNC_INITIALIZER(func_impl_name)<count> BOOST_PP_CAT(FUNC_INITIALIZER(func_impl_name),    \
                                                              _autorunner);
