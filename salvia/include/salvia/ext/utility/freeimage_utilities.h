...
FIBITMAP *make_bitmap_copy(eflib::rect<size_t> &out_region, size_t dest_width, size_t dest_height,
...
template <class ColorType> struct FREE_IMAGE_UNIFORM_COLOR {};

template <> struct FREE_IMAGE_UNIFORM_COLOR<RGBQUAD> {
  using CompT = uint8_t;
  const CompT &r;
  const CompT &g;
  const CompT &b;
...
template <> struct FREE_IMAGE_UNIFORM_COLOR<RGBTRIPLE> {
  using CompT = uint8_t;
  const CompT &r;
  const CompT &g;
  const CompT &b;
...
template <> struct FREE_IMAGE_UNIFORM_COLOR<FIRGBF> {
  using CompT = float;
  const CompT &r;
  const CompT &g;
  const CompT &b;
...
template <> struct FREE_IMAGE_UNIFORM_COLOR<FIRGBAF> {
  using CompT = float;
  const CompT &r;
  const CompT &g;
  const CompT &b;
