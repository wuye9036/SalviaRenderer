...
using shader_constant_types =
    std::tuple<empty, bool, int8_t, uint8_t, int16_t, uint16_t, int32_t, uint32_t, int64_t,
               uint64_t, std::string, std::wstring, eflib::vec2, eflib::vec3, eflib::vec4,
               eflib::mat44, std::vector<eflib::mat44>>;

template <class T, class Tuple> struct index_in_tuple;

template <class T, class... Types> struct index_in_tuple<T, std::tuple<T, Types...>> {
  static const std::size_t value = 0;
};

template <class T, class U, class... Types> struct index_in_tuple<T, std::tuple<U, Types...>> {
  static const std::size_t value = 1 + index_in_tuple<T, std::tuple<Types...>>::value;
};

template <typename T> struct type_encode {
...
// reg functions
////////////////////////////////////////////
template <class T> inline bool assign_impl(voidptr lhs, const_voidptr rhs) {
  *(T *)lhs = *(const T *)rhs;
  return true;
}

template <> inline bool assign_impl<empty>(voidptr /*lhs*/, const_voidptr /*rhs*/) { return false; }
