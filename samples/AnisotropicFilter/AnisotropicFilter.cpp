...
float const CYLINDER_SEG_HALF_WIDTH = CYLINDER_SEGMENTS > 2
    ? tanf(eflib::radians(CYLINDER_SEG_ANGLE / 2.0f)) * CYLINDER_RADIUS
    : 2 * CYLINDER_RADIUS;
...
public:
  ...
  uint32_t num_output_attributes() const { return 2; }

  uint32_t output_attribute_modifiers(uint32_t index) const {
    switch (index) {
    case 0: return salviar::vs_output::am_linear;

    case 1: return salviar::vs_output::am_linear;

    case 2: return salviar::vs_output::am_linear;

    default: return salviar::vs_output::am_linear;
    }
  }
...
char const* plane_ps_code = "sampler samp;"
...
public:
  ...
  uint32_t num_output_attributes() const { return 1; }
...
public:
  ps_plane(sampler_ptr const& samp) : samp_(samp) {}
...
protected:
  void on_init() override {
    create_devices_and_targets(data_->screen_width,
                               data_->screen_height,
                               1,
                               pixel_format_color_rgba8,
                               pixel_format_color_rg32f);

    data_->renderer->set_viewport(data_->screen_vp);

    planar_mesh = create_planar(data_->renderer.get(),
    ...
    filter_params =
        decltype(filter_params){// MIP FILTER, FILTER QUALITY
    ...
    plane_tex =
        load_texture(data_->renderer.get(), plane_tex_path, salviar::pixel_format_color_rgba8);
  ...
  void on_frame() override {
    ...
    mat_perspective_fov(
        proj, static_cast<float>(HALF_PI), data_->screen_aspect_ratio, 0.1f, 100.0f);
    ...
    case salvia::utility::app_modes::test:
      ...
      break;
    case salvia::utility::app_modes::interactive: break;
    ...
    std::tie(desc.mip_filter, desc.mip_qual, desc.max_anisotropy, window_title) =
        filter_params[i_param];

    if (data_->gui) {
      data_->gui->main_window()->set_title(window_title);
    }
...
protected:
  ...
  std::vector<std::tuple<filter_type, mip_quality, int /*aniso*/, std::string /*name*/>>
      filter_params;
};
