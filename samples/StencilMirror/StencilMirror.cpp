...
char const* cup_vs_code = "float4x4 wvpMatrix; \r\n"
                          "float4   eyePos; \r\n"
                          "float4	  lightPos; \r\n"
                          "struct VSIn{ \r\n"
                          "	float4 pos: POSITION; \r\n"
...
char const* mirror_vs_code = "float4x4 wvpMatrix; \r\n"
...
protected:
  void on_init() override {
    ...
    mirror_mesh = create_planar(data_->renderer.get(),
                                mirror_norm,
                                mirror_start_pos,
  ...
  void draw_cup(bool reflect) {
    ...
    for (size_t i_mesh = 0; i_mesh < cup_mesh.size(); ++i_mesh) {
      ...
      shared_ptr<obj_material> mtl = dynamic_pointer_cast<obj_material>(cur_mesh->get_attached());
  ...
  void on_frame() override {
    ...
    profiling("Rendering", [this]() { profiling("InMirror", [&] { draw_cup(true); }); });
  }
