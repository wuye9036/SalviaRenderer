...
using namespace salvia::utility;
...
public:
  vs_mesh() : wv(mat44::identity()), proj(mat44::identity()) {
    declare_constant("WorldViewMat", wv);
    declare_constant("ProjMat", proj);
    declare_constant("LightPos", light_pos);
    declare_constant("EyePos", eye_pos);
...
constexpr auto BENCHMARK_FRAME_COUNT = eflib::is_debug_mode ? 3 : 300;
constexpr auto TEST_FRAME_COUNT = 3;
...
public:
  ...
  void on_frame() override {
    ...
    profiling("Rendering", [&]() {
      ...
      cpp_vs->set_constant("WorldViewMat", &wv);
      cpp_vs->set_constant("ProjMat", &proj);
      cpp_vs->set_constant("LightPos", &light_pos);
      cpp_vs->set_constant("EyePos", &eye);
