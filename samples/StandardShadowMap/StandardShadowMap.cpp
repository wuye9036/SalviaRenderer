...
static float const gaussian_weights[] = {0.027681f,
                                         0.111014f,
                                         0.027681f,
                                         0.111014f,
                                         0.445213f,
...
class gen_sm_cpp_ps : public cpp_pixel_shader {
  bool output_depth() const { return true; }
...
public:
  ...
  vec4 to_color(vec3 const& v) {
    return vec4((v.x() + 1.0f) / 2.0f, (v.y() + 1.0f) / 2.0f, (v.z() + 1.0f) / 2.0f, 1.0f);
  }
  ...
  bool shader_prog(const vs_output& in, ps_output& out) {
    ...
    if (dsamp_) {
      ...
      vec2 sm_coords[] = {sm_center + vec2(-sm_offset, -sm_offset),
    ...
    vec4 illum =
        ambient + (diffuse * illum_diffuse + specular * pow(illum_specular, shininess)) * occlusion;
...
protected:
  void on_init() override {
    ...
    sm_texture_ = data_->renderer->create_tex2d(
        data_->screen_width, data_->screen_height, 1, pixel_format_color_rg32f);
    ...
    plane_mesh = create_planar(data_->renderer.get(),
                               vec3(-3.0f, 0.0f, -3.0f),
                               vec3(6.0f, -1.0f, 0.0f),
                               vec3(0.0f, -1.0f, 6.0f),
                               1,
  ...
  void gen_sm() {
    ...
    profiling("BackBufferClearing", [&]() {
      profiling("Shadow", [&]() {
        data_->renderer->clear_depth_stencil(
            sm_texture_->subresource(0), clear_depth | clear_stencil, 1.0f, 0);
      });
    });
  ...
  void draw() {
    ...
    profiling("BackBufferClearing", [&]() {
      profiling("Scene", [&]() {
        ...
        data_->renderer->clear_depth_stencil(
            data_->ds_target, clear_depth | clear_stencil, 1.0f, 0);
      });
    });
    ...
    profiling("Rendering", [&]() {
      profiling("Scene", [&]() {
        ...
        for (size_t i_mesh = 0; i_mesh < cup_mesh.size(); ++i_mesh) {
          ...
          if (mtl->tex) {
            draw_ps_->set_sampler(_T("TexSampler"),
                                  data_->renderer->create_sampler(desc, mtl->tex));
          } else {
            draw_ps_->set_sampler(_T("TexSampler"), sampler_ptr());
          }
          draw_ps_->set_sampler(_T("DepthSampler"), sm_sampler_);
  ...
  void on_frame() override {
    ...
    mat_perspective_fov(
        proj, static_cast<float>(HALF_PI * 0.5f), data_->screen_aspect_ratio, 0.1f, 100.0f);
    ...
    mat_perspective_fov(
        proj, static_cast<float>(HALF_PI * 0.5f), data_->screen_aspect_ratio, 0.1f, 40.0f);
