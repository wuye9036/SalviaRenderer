...
char const* ps_code = "float4 color; \r\n"
...
public:
  ...
  bool shader_prog(const vs_output& in, ps_output& out) {
    ...
    if (lower_bound == -1) {
      ...
      out.color[0] =
          lerp(colors[lower_bound], colors[lower_bound + 1], (height - lower_value) / interval);
    }
...
protected:
  virtual void on_init() {
    ...
    vector<float> field;
    salvia::ext::resource::make_terrain_plasma(field, TERRAIN_SIZE, 0.5f);
    salvia::ext::resource::filter_terrain(field, TERRAIN_SIZE, 0.15f);

    terrain_tex =
        salvia::ext::resource::make_terrain_texture(data_->renderer.get(), field, TERRAIN_SIZE);
    {
      sampler_desc desc;
    ...
    plane = create_planar(data_->renderer.get(),
                          vec3(-TERRAIN_BLOCK_SIZE / 2.0f, 0.0f, -TERRAIN_BLOCK_SIZE / 2.0f),
                          vec3(0.5f, 0.0f, 0.0f),
                          vec3(0.0f, 0.0f, 0.5f),
                          TERRAIN_BLOCK_SIZE * 2,
                          TERRAIN_BLOCK_SIZE * 2,
                          false);
  ...
  void on_frame() override {
    ...
    profiling("Rendering", [&]() { plane->render(); });
  }
