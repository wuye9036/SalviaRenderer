...
public:
  ...
  virtual bool
  initialize(module_vmcode_impl* mod, module_context* ctxt, sasl::semantic::module_semantic* sem);
  ...
  @{ */
  virtual multi_value
  emit_bitwise_bin_op(operators op, multi_value const& lhs, multi_value const& rhs);
  ...
  virtual multi_value
  emit_call(cg_function const& fn, std::vector<multi_value> const& args, llvm::Value* exec_mask);
  /// @}

  /// @name Emit element extraction
  /// @{
  virtual multi_value
  emit_insert_val(multi_value const& lhs, multi_value const& idx, multi_value const& elem_value);
  virtual multi_value
  emit_insert_val(multi_value const& lhs, size_t index, multi_value const& elem_value);
  ...
  virtual multi_value
  emit_select(multi_value const& flag, multi_value const& v0, multi_value const& v1);
  ...
  virtual multi_value
  emit_clamp(multi_value const& v, multi_value const& min_v, multi_value const& max_v);
  ...
  multi_value create_constant_scalar(std::integral auto v, cg_type* tyinfo, builtin_types hint) {
    llvm::Value* ll_val = llvm::ConstantInt::get(llvm::IntegerType::get(context(), sizeof(v) * 8),
                                                 uint64_t(v),
                                                 std::is_signed_v<decltype(v)>);
    return create_scalar(ll_val, tyinfo, hint);
  }

  multi_value
  create_constant_scalar(std::floating_point auto v, cg_type* tyinfo, builtin_types hint) {
  ...
  multi_value
  create_value(cg_type* tyinfo, builtin_types hint, value_array const& v, value_kinds k, abis abi);
  ...
  virtual multi_value
  create_value_by_scalar(multi_value const& scalar, cg_type* tyinfo, builtin_types hint);
  /// @}
  ...
  void jump_cond(multi_value const& cond_v,
...
protected:
  ...
  multi_value emit_cmp(multi_value const& lhs,
...
public:
  multi_value emit_bin_ps_ta_sva(multi_value const& lhs,
                                 multi_value const& rhs,
  ...
  multi_value emit_bin_es_ta_sva(std::string const& scalar_external_intrin_name,
                                 ...
                                 multi_value const& rhs);
  multi_value emit_bin_es_ta_sva(multi_value const& lhs,
                                 multi_value const& rhs,
                                 binary_intrin_functor signed_sv_fn,
                                 binary_intrin_functor unsigned_sv_fn,
                                 binary_intrin_functor float_sv_fn);
...
protected:
  multi_value emit_bin_mm(multi_value const& lhs,
                          multi_value const& rhs,
  ...
  virtual multi_value emit_tex_grad_impl(multi_value const& samp,
                                         ...
                                         externals::id ps_intrin);
  virtual multi_value
  emit_tex_bias_impl(multi_value const& samp, multi_value const& coord, externals::id ps_intrin);
  virtual multi_value
  emit_tex_proj_impl(multi_value const& samp, multi_value const& coord, externals::id ps_intrin);
