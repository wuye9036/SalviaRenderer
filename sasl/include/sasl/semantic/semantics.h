...
public:
  // General
  module_semantic *owner() const { return owner_; }
  sasl::syntax_tree::node *associated_node() const { return assoc_node_; }
  symbol *associated_symbol() const { return assoc_symbol_; }

  // Type
  int tid() const { return tid_; }
  builtin_types value_builtin_type() const;
  sasl::syntax_tree::tynode *ty_proto() const;

  // Constant
  int64_t const_signed() const { return signed_constant_; }
  uint64_t const_unsigned() const { return unsigned_constant_; }
  double const_double() const { return double_constant_; }
  std::string const_string() const;

  // Expression and variable
  salvia::shader::semantic_value *semantic_value() const { return semantic_value_; }
  salvia::shader::semantic_value const &semantic_value_ref() const;
  salvia::shader::reg_name user_defined_reg() const { return user_defined_reg_; }
  int member_index() const { return member_index_; }
  elem_indexes swizzle() const { return swizzle_code_; }
  bool is_reference() const { return is_reference_; }
  bool is_function_pointer() const { return is_function_pointer_; }
  lvalue_or_rvalue::id lr_value() const { return lrv_; }
  ...
  sasl::syntax_tree::node *referenced_declarator() const { return referenced_declarator_; }
  // Statement
  labeled_statement_array const &labeled_statements() const;
