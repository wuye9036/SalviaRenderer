...
void semantic_cases::initialize(){
	CHECK_RET( LOCVAR_(si_root) = SEMANTIC_(analysis_semantic)( SYNCASE_(prog_for_semantic_test), NULL, 1/*lang_general*/ ) );
	CHECK_RET( LOCVAR_(sym_root) = LOCVAR_(si_root)->root_symbol() );
	...
	CHECK_RET( LOCVAR_(fn2_sem) = sym_fn2_sem()->node()->as_handle<function_type>() );

	CHECK_RET( LOCVAR_(fn0_sem) = sym_fn0_sem()->node()->as_handle< SYNTAX_(function_type) >() );
	CHECK_RET( LOCVAR_(fn1_sem) = sym_fn1_sem()->node()->as_handle< SYNTAX_(function_type) >() );
	...
	CHECK_RET( LOCVAR_(par0_0_fn1) = LOCVAR_(sym_par0_0_fn1)->node()->as_handle< SYNTAX_(parameter) >() );
	CHECK_RET( LOCVAR_(par1_1_fn1) = LOCVAR_(sym_par1_1_fn1)->node()->as_handle< SYNTAX_(parameter) >() );

	CHECK_RET( LOCVAR_(body_fn2) = fn2_sem()->body );
	CHECK_RET( LOCVAR_(sym_body_fn2) = body_fn2()->symbol() );
	CHECK_RET( LOCVAR_(jstmt0_0_fn2) = body_fn2()->stmts[0]->as_handle< SYNTAX_(jump_statement) >() );
	CHECK_RET( LOCVAR_(bexpr0_0_jstmts0) = jstmt0_0_fn2()->jump_expr->as_handle< SYNTAX_(binary_expression) >() );
	CHECK_RET( LOCVAR_(cexpr0_l_bexpr0) = bexpr0_0_jstmts0()->left_expr->as_handle< SYNTAX_(constant_expression) >() );
