...
BOOST_FIXTURE_TEST_CASE(intrinsics, jit_fixture) {
  ...
  }
  {
    ...
    vec4 ref1 =
        vec4(fmodf(v0[0], v1[0]), fmodf(v0[1], v1[1]), fmodf(v0[2], v1[2]), fmodf(f, v1[1]));
  ...
  {
    float arr0[3][4] = {{17.7f, 66.3f, 0.92f, -88.7f},
                        {8.6f, -0.22f, 17.1f, -64.4f},
                        {199.8f, 0.1f, -0.1f, 99.73f}};

    float arr1[3][4] = {{9.62f, 10.33f, -18.2f, 99.7f},
    ...
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < 4; ++j) {
        ...
        ret.f = ret_sin.data_[i][j];
        ref.f = sinf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_cos.data_[i][j];
        ref.f = cosf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_tan.data_[i][j];
        ref.f = tanf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_sinh.data_[i][j];
        ref.f = sinhf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_cosh.data_[i][j];
        ref.f = coshf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_tanh.data_[i][j];
        ref.f = tanhf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_asin.data_[i][j];
        ref.f = asinf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_acos.data_[i][j];
        ref.f = acosf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_atan.data_[i][j];
        ref.f = atanf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_ceil.data_[i][j];
        ref.f = fast_ceil(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_floor.data_[i][j];
        ref.f = fast_floor(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_round.data_[i][j];
        ref.f = fast_round(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_trunc.data_[i][j];
        ref.f = trunc(arr0[i][j]);
        BOOST_CHECK_EQUAL(ret.f, ref.f);
        ret.f = ret_log.data_[i][j];
        ref.f = fast_log(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_log2.data_[i][j];
        ref.f = fast_log2(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_log10.data_[i][j];
        ref.f = log10f(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_rsqrt.data_[i][j];
        ref.f = 1.0f / sqrtf(arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_rcp.data_[i][j];
        ref.f = 1.0f / arr0[i][j];
        BOOST_CHECK_CLOSE(ret.f, ref.f, RELATIVE_TORLERANCE_NORMAL);
        ret.f = ret_pow.data_[i][j];
        ref.f = powf(arr0[i][j], arr1[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);

        // TODO: ldexp and exp2 are not followed HLSL spec. Should fix them later.
        ret.f = ret_exp2.data_[i][j];
        ref.f = ldexp(1.0f, (int)arr0[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
        ret.f = ret_ldexp.data_[i][j];
        ref.f = ldexpf(arr0[i][j], (int)arr1[i][j]);
        BOOST_CHECK_BITWISE_EQUAL(ret.u, ref.u);
      }
    }
  }
  {
    ...
    float m[2][3] = {{17.7f, 66.3f, 0.92f}, {-88.7f, 8.6f, -0.22f}};

    float min_m[2][3] = {{17.1f, -64.4f, 199.8f}, {0.1f, -0.1f, 99.73f}};

    float max_m[2][3] = {{9.62f, 10.33f, -18.2f}, {99.7f, -0.3f, -76.9f}};
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 3; ++j) {
        BOOST_CHECK_EQUAL(ret0.data_[i][j], clamp(m[i][j], min_m[i][j], max_m[i][j]));
        BOOST_CHECK_EQUAL(ret1.data_[i][j], std::min(m[i][j], max_m[i][j]));
        BOOST_CHECK_EQUAL(ret2.data_[i][j], std::max(m[i][j], max_m[i][j]));
        BOOST_CHECK_EQUAL(ret3.data_[i][j], clamp(m[i][j], 0.0f, 1.0f));
        BOOST_CHECK_EQUAL(ret4.data_[i][j], max_m[i][j] * min_m[i][j] + m[i][j]);
        BOOST_CHECK_EQUAL(ret5.data_[i][j], min_m[i][j] <= m[i][j] ? 1.0f : 0.0f);
        BOOST_CHECK_CLOSE(ret6.data_[i][j], smoothstep(min_m[i][j], max_m[i][j], m[i][j]), 0.0002f);
      }
    }
  }
  {
  ...
  }
  {
    ...
    int32_t ref4[3] = {static_cast<int32_t>(naive_reversebits(static_cast<uint32_t>(v0[0]))),
    ...
    uint32_t ref5[2] = {naive_reversebits(v1[0]), naive_reversebits(v1[1])};
...
BOOST_FIXTURE_TEST_CASE(bool_test, jit_fixture) {
  ...
  {
    float array0[3][4] = {{17.7f, 66.3f, 0.92f, -88.7f},
                          {8.6f, -0.22f, 17.1f, -64.4f},
                          {199.8f, 0.1f, -0.1f, 99.73f}};

    float array1[3][4] = {{9.62f, 10.33f, -18.2f, 99.7f},
                          {-0.3f, -76.9f, 93.3f, 0.22f},
                          {44.1f, 0.027f, 19.9f, -33.5f}};

    float array2[3][4] = {{0.198f, 10.3f, -0.82f, 9.37f},
...
BOOST_FIXTURE_TEST_CASE(cast_tests, jit_fixture) {
  ...
  {
    uint32_t uarr[2][3] = {{786, 0, 33769097}, {0xFFFFFFFF, 3899927, 67}};
...
template <typename T, typename MemberPtr> void get_ddx(T *out, T const *in, MemberPtr ptr) {
  ...
  for (int col = 0; col < PACKAGE_LINE_ELEMENT_COUNT; col += 2) {
    for (int row = 0; row < LINES; ++row) {
      ...
      out[index + 1].*ptr = out[index].*ptr = in[index + 1].*ptr - in[index].*ptr;
    }
  }
}
...
template <typename T, typename MemberPtr> void get_ddy(T *out, T const *in, MemberPtr ptr) {
  ...
  for (int row = 0; row < LINES; row += 2) {
    for (int col = 0; col < PACKAGE_LINE_ELEMENT_COUNT; ++col) {
      ...
      out[next_row_index].*ptr = out[index].*ptr = in[next_row_index].*ptr - in[index].*ptr;
    }
  }
}
...
BOOST_FIXTURE_TEST_CASE(arith_ops, jit_fixture) {
...

  {
    ...
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < 4; ++j) {
        ref_v[i][j] =
            (arr0[i][j] + arr1[i][j]) * arr0[i][j] - fmodf((arr1[i][j] / arr0[i][j]), arr1[i][j]);
      }
    }
...
BOOST_FIXTURE_TEST_CASE(assigns, jit_fixture) {
  ...
  int32_t lhs_arr[2][3] = {{786, 0, 33769097}, {-1, -3899927, 67}};
...
BOOST_FIXTURE_TEST_CASE(array_test, jit_fixture) {
  ...
  //   printf("sin.ids:        %08llx\n", reinterpret_cast<std::uintptr_t>(sin.ids) &
  //   LOW_BITS_MASK); printf("sin.pos:        %08llx\n", reinterpret_cast<std::uintptr_t>(sin.pos)
  //   & LOW_BITS_MASK); printf("&bin:           %08llx\n", reinterpret_cast<std::uintptr_t>(&bin) &
  //   LOW_BITS_MASK); printf("bin.mat_arr:    %08llx\n",
