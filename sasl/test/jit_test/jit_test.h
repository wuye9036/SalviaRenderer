...
template <typename Head, typename... Ts> struct cat_type_list<Head, type_list<Ts...>> {
  typedef type_list<Head, Ts...> type;
};

template <typename RetT, typename ParamListT> struct make_function;

template <typename RetT, typename... ParamTs> struct make_function<RetT, type_list<ParamTs...>> {
  using type = void(RetT, ParamTs...);
};

template <typename... ParamTs> struct make_function<void, type_list<ParamTs...>> {
  using type = void(ParamTs...);
};

template <typename Conv, typename... Ts> struct convert_types;

template <typename Conv> struct convert_types<Conv> {
  typedef type_list<> type;
};

template <typename Conv, typename Head, typename... Ts> struct convert_types<Conv, Head, Ts...> {
  typedef typename cat_type_list<typename boost::mpl::apply<Conv, Head>::type,
                                 typename convert_types<Conv, Ts...>::type>::type type;
};

template <typename Conv, typename Head> struct convert_types<Conv, Head> {
  typedef type_list<typename boost::mpl::apply<Conv, Head>::type> type;
};

template <typename Conv, typename FuncT> struct convert_to_jit_function_type {};
...
template <typename Fn> class jit_function_forward_base {
public:
...
template <typename Fn> class jit_function_forward : public jit_function_forward_base<Fn> {
public:
  ...
  template <typename... Ts> return_type operator()(Ts... params) {
    return_type tmp;
#if defined(EFLIB_MSVC)
    __try {
      callee(&tmp, params...);
...
template <typename Fn> class jit_function : public jit_function_forward<Fn> {};
...
public:
  ...
  template <typename FunctionT> void function(FunctionT &fn, string const &unmangled_name) {
