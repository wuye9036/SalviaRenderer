...
BOOST_FIXTURE_TEST_CASE(semantic_fn_svs, abi_test_fixture) {
  ...
  auto out_pos_reg =
      reflection2->find_reg(reg_categories::outputs, semantic_value("SV_Position", 0));
  ...
  auto in_pos_reg =
      reflection2->find_reg(reg_categories::varying, semantic_value("SV_Position", 0));
  BOOST_CHECK(!in_pos_reg.valid());

  auto global_pos_reg =
      reflection2->find_reg(reg_categories::uniforms, semantic_value("SV_Position", 0));
