  ...
  if (driver_sc) {
    ...
    if (user_inc_handler) {
      ...
      for (size_t i = 0; i < macros.size(); ++i) {
        ...
        switch (ms) {
        case ms_normal: driver_sc->add_macro(macros[i].first, false); break;
        case ms_predef: driver_sc->add_macro(macros[i].first, true); break;
        case ms_remove: driver_sc->remove_macro(macros[i].first); break;
        }
      }
...
shared_ptr<diag_chat> compiler_impl::compile(vector<external_function_desc> const& external_funcs,
  ...
  for (size_t i = 0; i < external_funcs.size(); ++i) {
    inject_function(
        external_funcs[i].func, external_funcs[i].func_name, external_funcs[i].is_raw_name);
  }
...
void sasl_exp_f32(float* ret, float v) {
  *ret = expf(v);
}
void sasl_exp2_f32(float* ret, float v) {
  *ret = ldexpf(1.0f, static_cast<int>(v));
}
void sasl_sin_f32(float* ret, float v) {
  *ret = sinf(v);
}
void sasl_cos_f32(float* ret, float v) {
  *ret = cosf(v);
}
void sasl_tan_f32(float* ret, float v) {
  *ret = tanf(v);
}
void sasl_sinh_f32(float* ret, float v) {
  *ret = sinhf(v);
}
void sasl_cosh_f32(float* ret, float v) {
  *ret = coshf(v);
}
void sasl_tanh_f32(float* ret, float v) {
  *ret = tanhf(v);
}
void sasl_asin_f32(float* ret, float v) {
  *ret = asinf(v);
}
void sasl_acos_f32(float* ret, float v) {
  *ret = acosf(v);
}
void sasl_atan_f32(float* ret, float v) {
  *ret = atanf(v);
}
void sasl_ceil_f32(float* ret, float v) {
  *ret = eflib::fast_ceil(v);
}
void sasl_floor_f32(float* ret, float v) {
  *ret = eflib::fast_floor(v);
}
void sasl_round_f32(float* ret, float v) {
  *ret = eflib::fast_round(v);
}
void sasl_trunc_f32(float* ret, float v) {
  *ret = eflib::trunc(v);
}
void sasl_log_f32(float* ret, float v) {
  *ret = eflib::fast_log(v);
}
void sasl_log10_f32(float* ret, float v) {
  *ret = log10f(v);
}
void sasl_log2_f32(float* ret, float v) {
  *ret = eflib::fast_log2(v);
}
void sasl_rsqrt_f32(float* ret, float v) {
  *ret = 1.0f / sqrtf(v);
}
void sasl_mod_f32(float* ret, float lhs, float rhs) {
  *ret = fmodf(lhs, rhs);
}
void sasl_ldexp_f32(float* ret, float lhs, float rhs) {
  *ret = ldexpf(lhs, static_cast<int>(rhs));
}
void sasl_pow_f32(float* ret, float lhs, float rhs) {
  *ret = powf(lhs, rhs);
}
...
void compiler_impl::add_virtual_file(string const& file_name,
                                     string const& code_content,
                                     bool high_priority) {
...
shader_reflection2_ptr compiler_impl::get_reflection2() const {
  return reflection2_;
}
