...
symbol::symbol_array symbol::find_overloads(string_view unmangled, caster_t *conv,
...
symbol::symbol_array symbol::find_assign_overloads(string_view unmangled, caster_t *conv,
                                                   expression_array const &args) const {
...
symbol *symbol::parent() const { return parent_; }

node *symbol::associated_node() const { return associated_node_; }

void symbol::associated_node(node *v) { associated_node_ = v; }
  ...
  for (symbol *cand : candidates) {
    ...
    for (symbol *filterated : ret) {
      ...
      for (size_t i_param = 0; i_param < param_count; ++i_param) {
        bool same = false;
        bool equiva = false;
        is_same_or_equiva(owner_, cand_fn->params[i_param].get(),
                          filterated_fn->params[i_param].get(), same, equiva);
        if (!(same || equiva)) {
          same_function = false;
          break;
        }
...
symbol::symbol_array symbol::find_overloads_impl(string_view unmangled, caster_t *conv,
                                                 expression_array const &args) const {
  // find all overloads
  ...
  for (size_t i_cand = 0; i_cand < candidates.size(); ++i_cand) {
    ...
    for (size_t j_cand = i_cand + 1; j_cand < candidates.size(); ++j_cand) {
      ...
      for (size_t i_param = 0; i_param < args.size(); ++i_param) {
        ...
        conv->better_or_worse(matched_par_type, matching_par_type, arg_type, par_is_better,
                              par_is_worse);
        if (par_is_better) {
          ++better_param_count;
        }
...
module_semantic *symbol::owner() const { return owner_; }
