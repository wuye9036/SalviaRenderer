...
SASL_VISIT_DEF(binary_expression) {
  ...
  if (v.op == operators::logic_and || v.op == operators::logic_or) {
    ...
    if (/**/ v.op == operators::assign || v.op == operators::add_assign ||
        v.op == operators::sub_assign || v.op == operators::mul_assign ||
        v.op == operators::div_assign || v.op == operators::mod_assign ||
        v.op == operators::lshift_assign || v.op == operators::rshift_assign ||
        v.op == operators::bit_and_assign || v.op == operators::bit_or_assign ||
...
SASL_SPECIFIC_VISIT_DEF(visit_return, jump_statement) {
  ...
  if (!v.jump_expr) {
    ...
    service()->emit_return(node_ctxt(v.jump_expr)->node_value,
                           service()->param_abi(service()->fn().c_compatible));
  }
}
...
SASL_SPECIFIC_VISIT_DEF(process_intrinsics, program) {
  ...
  for (symbol *intr : intrinsics) {
    ...
    if (intr->unmangled_name() == "mul") {
      ...
      multi_value ret_val =
          service()->emit_mul_intrin(service()->fn().arg(0), service()->fn().arg(1));
      ...
      service()->emit_return(ret_val, service()->param_abi(false));
    } else if (intr->unmangled_name() == "exp" || intr->unmangled_name() == "exp2" ||
               intr->unmangled_name() == "sin" || intr->unmangled_name() == "cos" ||
               intr->unmangled_name() == "tan" || intr->unmangled_name() == "asin" ||
               intr->unmangled_name() == "acos" || intr->unmangled_name() == "atan" ||
               intr->unmangled_name() == "ceil" || intr->unmangled_name() == "floor" ||
               intr->unmangled_name() == "round" || intr->unmangled_name() == "trunc" ||
               intr->unmangled_name() == "log" || intr->unmangled_name() == "log2" ||
               intr->unmangled_name() == "log10" || intr->unmangled_name() == "rsqrt" ||
               intr->unmangled_name() == "sinh" || intr->unmangled_name() == "cosh" ||
               intr->unmangled_name() == "tanh") {
      assert(par_tys.size() == 1);
      service()->fn().arg_name(0, ".value");
      std::string scalar_intrin_name = fmt::format("sasl.{}.f32", intr->unmangled_name());
      multi_value ret_val = service()->emit_unary_ps(scalar_intrin_name, service()->fn().arg(0));
      service()->emit_return(ret_val, service()->param_abi(false));
    } else if (intr->unmangled_name() == "countbits" || intr->unmangled_name() == "count_bits") {
      assert(par_tys.size() == 1);
      service()->fn().arg_name(0, ".value");
      multi_value ret_val = service()->emit_unary_ps("sasl.countbits.u32", service()->fn().arg(0));
      service()->emit_return(ret_val, service()->param_abi(false));
    } else if (intr->unmangled_name() == "firstbithigh" ||
               intr->unmangled_name() == "firstbitlow" || intr->unmangled_name() == "reversebits") {
      assert(par_tys.size() == 1);
      service()->fn().arg_name(0, ".value");
      multi_value ret_val = service()->emit_unary_ps(
          fmt::format("sasl.{}.u32", intr->unmangled_name()), service()->fn().arg(0));
      service()->emit_return(ret_val, service()->param_abi(false));
    } else if (intr->unmangled_name() == "ldexp" || intr->unmangled_name() == "pow") {
      assert(par_tys.size() == 2);
      service()->fn().arg_name(0, ".lhs");
      service()->fn().arg_name(1, ".rhs");
      std::string scalar_intrin_name = fmt::format("sasl.{}.f32", intr->unmangled_name());
      multi_value ret_val = service()->emit_bin_ps_ta_sva(
          scalar_intrin_name, service()->fn().arg(0), service()->fn().arg(1));
      service()->emit_return(ret_val, service()->param_abi(false));
        ...
        for (size_t i_arg = 0; i_arg < fn.logical_args_count(); ++i_arg) {
          ...
          if (is_scalar(arg_hint)) {
            ...
            for (size_t i_scalar_in_arg = 0; i_scalar_in_arg < arg_vec_size; ++i_scalar_in_arg) {
              multi_value scalar_value =
                  service()->emit_extract_val(arg_value, static_cast<int>(i_scalar_in_arg));
      ...
      multi_value t =
          service()->emit_div(service()->emit_sub(v, min_v), service()->emit_sub(max_v, min_v));
      t = service()->emit_saturate(t);

      multi_value two = service()->numeric_value(t, 2.0, 2);
      multi_value three = service()->numeric_value(t, 3.0, 3);

      multi_value ret = service()->emit_sub(three, service()->emit_mul_comp(two, t));
      ...
      multi_value diffuse =
          service()->emit_select(service()->emit_cmp_lt(n_dot_l, zero), zero, n_dot_l);
