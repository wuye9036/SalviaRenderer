...
multi_value cg_sisd::emit_short_cond(shared_ptr<node> const &cond, shared_ptr<node> const &yes,
  ...
  visit_child(cond);
  multi_value cond_value = node_ctxt(cond)->node_value.to_rvalue();
...
SASL_VISIT_DEF(cond_expression) {
  EFLIB_UNREF_DECLARATOR(data);
  node_ctxt(v, true)->node_value = emit_short_cond(v.cond_expr, v.yes_expr, v.no_expr);
}
...
SASL_VISIT_DEF(unary_expression) {
  ...
  if (v.op == operators::negative) {
    ...
    multi_value all_one_value =
        service()->create_constant_int(nullptr, hint, inner_value.abi(), 0xFFFFFFFFFFFFFFFF);
    ctxt->node_value =
    ...
    if (v.op == operators::prefix_incr) {
      multi_value inc_v = service()->emit_add(inner_value, one_value);
      inner_value.store(inc_v);
      ctxt->node_value = inner_value;
    } else if (v.op == operators::prefix_decr) {
      multi_value dec_v = service()->emit_sub(inner_value, one_value);
...
SASL_VISIT_DEF(compound_statement) {
  ...
  for (std::vector<std::shared_ptr<statement>>::iterator it = v.stmts.begin(); it != v.stmts.end();
       ++it) {
    visit_child(*it);
  }
}
...
SASL_VISIT_DEF(if_statement) {
  ...
  multi_value cond_value = node_ctxt(v.cond, false)->node_value;
...
SASL_VISIT_DEF(switch_statement) {
  ...
  for (weak_ptr<labeled_statement> const &weak_lbl_stmt : ssi->labeled_statements()) {
    ...
    for (shared_ptr<label> const &lbl : lbl_stmt->labels) {
      ...
      if (case_lbl->expr) {
        multi_value v = node_ctxt(case_lbl->expr)->node_value;
  ...
  // Fill back jumps
  service()->set_insert_point(cond_end);
  multi_value cond_v = node_ctxt(v.cond)->node_value;
