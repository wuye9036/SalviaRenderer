...
public:
  ...
  void int2int(node *dest, node *src) {
    ...
    multi_value casted = cgs->cast_ints(src_ctxt->node_value.to_rvalue(), dest_ctxt->ty);
  ...
  void int2float(node *dest, node *src) {
    ...
    multi_value casted = cgs->cast_i2f(src_ctxt->node_value.to_rvalue(), dest_ctxt->ty);
    store(dest, src, casted);
  }
  ...
  void float2int(node *dest, node *src) {
    ...
    multi_value casted = cgs->cast_f2i(src_ctxt->node_value.to_rvalue(), dest_ctxt->ty);
    store(dest, src, casted);
  }
  ...
  void float2float(node *dest, node *src) {
    ...
    multi_value casted = cgs->cast_f2f(src_ctxt->node_value.to_rvalue(), dest_ctxt->ty);
  ...
  void shrink_vector(node *dest, node *src, int source_size, int dest_size) {
    ...
    cgs->store(dest_ctxt->node_value, vector_value.swizzle(elem_indexes::from_length(dest_size)));
  }
  ...
  for (int src_size = 1; src_size < 5; ++src_size) {
    for (int dest_size = 0; dest_size < 5; ++dest_size) {
      if (src_size > dest_size) {
        shrink_vec_pfn[src_size][dest_size] =
            bind(&cg_caster::shrink_vector, cst.get(), _1, _2, src_size, dest_size);
      }
    }
  }
