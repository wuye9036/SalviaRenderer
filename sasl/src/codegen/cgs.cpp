...
using sasl::enums::is_integer;
using sasl::enums::is_matrix;
using sasl::enums::is_real;
using sasl::enums::is_sampler;
using sasl::enums::is_scalar;
using sasl::enums::is_signed;
using sasl::enums::is_vector;
using sasl::enums::matrix_of;
using sasl::enums::replace_scalar;
using sasl::enums::row_vector_of;
using sasl::enums::scalar_of;
using sasl::enums::storage_size;
...
value_array cg_service::load_ref(multi_value const& v) {
  ...
  case static_cast<int>(value_kinds::elements) | static_cast<int>(value_kinds::reference): {
  ...
  for (size_t value_index = 0; value_index < parallel_factor_; ++value_index) {
    ...
    if (value_type->isStructTy()) {
      ...
      new_value[value_index] =
          builder().CreateInsertElement(agg[value_index], elem[value_index], indexes[value_index]);
    }
  }
...
multi_value cg_service::emit_div(multi_value const& lhs, multi_value const& rhs) {
  ...
  binary_intrin_functor u_div =
      std::bind(&DefaultIRBuilder::CreateUDiv, std::ref(builder()), _1, _2, "", false);
...
multi_value cg_service::emit_extract_elem_mask(multi_value const& vec,
  ...
  if (vec.storable()) {
    multi_value swz_proxy =
        create_value(nullptr, swz_hint, invalid_value_array(), value_kinds::elements, vec.abi());
...
multi_value cg_service::emit_abs(multi_value const& arg_value) {
  ...
  value_array ret_v =
      ext_->call_unary_intrin(v[0]->getType(), v, std::bind(&cg_extension::abs_sv, ext_.get(), _1));
...
multi_value cg_service::emit_bin_ps_ta_sva(std::string const& scalar_external_intrin_name,
  ...
  value_array ret_v =
      ext_->call_binary_intrin(nullptr, v0.load(abi), v1.load(abi), intrin_sv, null_unary);
...
multi_value cg_service::emit_tex_grad_impl(multi_value const& samp,
  ...
  value_array args[] = {ret_ptr, masks, samp.load(), coord_ptr, ddx_ptr, ddy_ptr};
