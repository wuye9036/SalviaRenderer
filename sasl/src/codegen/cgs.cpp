...
cg_service::cg_service(size_t parallel_factor)
    : vmcode_(nullptr), ctxt_(nullptr), sem_(nullptr),
      parallel_factor_(parallel_factor) {}

bool cg_service::initialize(module_vmcode_impl *mod, module_context *ctxt,
                            module_semantic *sem) {
  assert(mod);
  assert(ctxt);
  assert(sem);
...
cg_function *cg_service::fetch_function(function_def *fn_node) {
  ...
  Type *ret_ty =
      ctxt_->get_node_context(fn_node->type->result_type.get())->ty->ty(abi);
  ...
  ret->fn->addFnAttr(
      Attribute::getWithStackAlignment(context(), Align(32)).getAsString());
...
multi_value cg_service::create_value(cg_type *tyinfo, value_array const &values,
                                     value_kinds k, abis abi) {
  return multi_value(tyinfo, values, k, abi, this);
}

multi_value cg_service::create_value(builtin_types hint,
                                     value_array const &values, value_kinds k,
                                     abis abi) {
  return multi_value(hint, values, k, abi, this);
}

multi_value cg_service::create_value(cg_type *tyinfo, builtin_types hint,
                                     value_array const &values, value_kinds k,
                                     abis abi) {
  if (tyinfo) {
...
cg_type *cg_service::create_ty(tynode *tyn) {
  ...
  if (tyn->is_builtin()) {
    ret->vm_type(abis::c, type_(tyn->tycode, abis::c));
    ret->vm_type(abis::llvm, type_(tyn->tycode, abis::llvm));
    ret->vm_type(abis::regs, type_(tyn->tycode, abis::regs));
  } else {
    if (tyn->is_struct()) {
      ...
      for (shared_ptr<declaration> const &decl : struct_tyn->decls) {
        if (decl->node_class() == node_ids::variable_declaration) {
          shared_ptr<variable_declaration> decl_tyn =
              decl->as_handle<variable_declaration>();
          cg_type *decl_cgty =
              create_ty(sem_->get_semantic(decl_tyn->type_info)->ty_proto());
          size_t declarator_count = decl_tyn->declarators.size();
          c_member_types.insert(c_member_types.end(), declarator_count,
                                decl_cgty->ty(abis::c));
      ...
      StructType *ty_c = StructType::create(
          c_member_types, struct_tyn->name->str.raw_string() + ".abi.c");
      StructType *ty_llvm = StructType::create(
          llvm_member_types, struct_tyn->name->str.raw_string() + ".abi.llvm");
...
cg_type *cg_service::member_tyinfo(cg_type const *agg, size_t index) const {
  if (!agg) {
    ...
    shared_ptr<struct_type> struct_sty =
        agg->tyn_ptr()->as_handle<struct_type>();
    ...
    for (auto const &child : struct_sty->decls) {
      if (child->node_class() == node_ids::variable_declaration) {
        ...
        if (index < var_index) {
          return const_cast<cg_service *>(this)
              ->ctxt_->get_node_context(vardecl.get())
              ->ty;
        }
      }
    }
...
multi_value cg_service::create_variable(builtin_types bt, abis abi,
...
multi_value cg_service::create_variable(cg_type const *ty, abis abi,
                                        std::string const &name) {
  ...
  return create_value(const_cast<cg_type *>(ty), values, value_kinds::reference,
                      abi);
}

insert_point_t cg_service::new_block(std::string const &hint,
                                     bool set_as_current) {
  assert(in_function());
  insert_point_t ret;
...
value_array cg_service::load(multi_value const &v) {
  ...
  value_array ret = value_array(v.value_count(), nullptr);
  ...
  if (kind == value_kinds::reference || kind == value_kinds::value) {
    ...
    if (mem_indexes.length() > 0) {
      ...
      if (mem_indexes.length() == 1 && is_scalar(v.hint())) {
        ...
        assert((kind & value_kinds::reference) == value_kinds::unknown);
        multi_value resolved =
            emit_extract_elem_mask(v.parent()->to_rvalue(), mem_indexes);
  ...
  // Resolve reference
  if ((kind & value_kinds::reference) != value_kinds::unknown) {
    for (size_t i = 0; i < v.value_count(); ++i) {
      ret[i] = builder().CreateLoad(ret[i]->getType(), ret[i]);
    }
  }
...
value_array cg_service::load_ref(multi_value const &v) {
  ...
  case static_cast<int>(value_kinds::elements) |
      static_cast<int>(value_kinds::reference): {
...
multi_value cg_service::emit_insert_val(multi_value const &lhs,
  ...
  return create_value(lhs.ty(), lhs.hint(), new_value, value_kinds::value,
                      lhs.abi());
}
  ...
  for (size_t value_index = 0; value_index < parallel_factor_; ++value_index) {
    ...
    if (value_type->isStructTy()) {
      new_value[value_index] = builder().CreateInsertValue(
          agg[value_index], elem_vm_values[value_index],
      ...
      multi_value index_value =
          create_value(builtin_types::_sint32, index_vm_values,
...
multi_value cg_service::emit_mul_comp(multi_value const &lhs,
  ...
  if (lhint != rhint) {
    if (is_scalar(lhint)) {
      lv = extend_to_vm(lhs, rhint);
    } else if (is_scalar(rhint)) {
      rv = extend_to_vm(rhs, lhint);
    } else {
      assert(false);
    }
  }

  binary_intrin_functor f_mul =
      std::bind(&DefaultIRBuilder::CreateFMul, std::ref(builder()), _1, _2, "",
                (llvm::MDNode *)(nullptr));
  binary_intrin_functor i_mul =
      std::bind(&DefaultIRBuilder::CreateMul, std::ref(builder()), _1, _2, "",
                false, false);
...
multi_value cg_service::emit_add(multi_value const &lhs,
                                 multi_value const &rhs) {
  binary_intrin_functor f_add =
      std::bind(&DefaultIRBuilder::CreateFAdd, std::ref(builder()), _1, _2, "",
                (llvm::MDNode *)(nullptr));
  binary_intrin_functor i_add =
      std::bind(&DefaultIRBuilder::CreateAdd, std::ref(builder()), _1, _2, "",
                false, false);

  return emit_bin_es_ta_sva(lhs, rhs, i_add, i_add, f_add);
}

multi_value cg_service::emit_sub(multi_value const &lhs,
                                 multi_value const &rhs) {
  binary_intrin_functor f_sub =
      std::bind(&DefaultIRBuilder::CreateFSub, std::ref(builder()), _1, _2, "",
                (llvm::MDNode *)(nullptr));
  binary_intrin_functor i_sub =
      std::bind(&DefaultIRBuilder::CreateSub, std::ref(builder()), _1, _2, "",
                false, false);

  return emit_bin_es_ta_sva(lhs, rhs, i_sub, i_sub, f_sub);
}

multi_value cg_service::emit_mul_intrin(multi_value const &lhs,
                                        multi_value const &rhs) {
  builtin_types lhint = lhs.hint();
  builtin_types rhint = rhs.hint();
...
multi_value cg_service::emit_div(multi_value const &lhs,
                                 multi_value const &rhs) {
  binary_intrin_functor f_div =
      std::bind(&DefaultIRBuilder::CreateFDiv, std::ref(builder()), _1, _2, "",
                (llvm::MDNode *)(nullptr));
  binary_intrin_functor i_div = std::bind(
      &DefaultIRBuilder::CreateSDiv, std::ref(builder()), _1, _2, "", false);
  binary_intrin_functor u_div = std::bind(
      &DefaultIRBuilder::CreateUDiv, std::ref(builder()), _1, _2, "", false);
  binary_intrin_functor i_safe_div =
      std::bind(&cg_extension::safe_idiv_imod_sv, ext_.get(), _1, _2, i_div);
  binary_intrin_functor u_safe_div =
      std::bind(&cg_extension::safe_idiv_imod_sv, ext_.get(), _1, _2, u_div);

  return emit_bin_es_ta_sva(lhs, rhs, i_safe_div, u_safe_div, f_div);
}

multi_value cg_service::emit_mod(multi_value const &lhs,
                                 multi_value const &rhs) {
  binary_intrin_functor i_mod =
      std::bind(&DefaultIRBuilder::CreateSRem, std::ref(builder()), _1, _2, "");
  binary_intrin_functor u_mod =
      std::bind(&DefaultIRBuilder::CreateURem, std::ref(builder()), _1, _2, "");

  binary_intrin_functor i_safe_mod =
      std::bind(&cg_extension::safe_idiv_imod_sv, ext_.get(), _1, _2, i_mod);
  binary_intrin_functor u_safe_mod =
      std::bind(&cg_extension::safe_idiv_imod_sv, ext_.get(), _1, _2, u_mod);

  binary_intrin_functor intrin_mod_f32 =
      ext_->bind_external_to_binary(externals::mod_f32);
  ...
  binary_intrin_functor f_mod =
      std::bind(&cg_extension::call_binary_intrin_mono, ext_.get(),
                (Type *)nullptr, _1, _2, f_mod_sv, null_unary);

  return emit_bin_es_ta_sva(lhs, rhs, i_safe_mod, u_safe_mod, f_mod);
}

multi_value cg_service::emit_bitwise_bin_op(operators op,
                                            multi_value const &lhs,
                                            multi_value const &rhs) {
  auto vm_op = static_cast<Instruction::BinaryOps>(conv_bin_op_to_vm_[op]);
  binary_intrin_functor bin_op =
      std::bind(&DefaultIRBuilder::CreateBinOp, std::ref(builder()), vm_op, _1,
                _2, "", nullptr);
  return emit_bin_es_ta_sva(lhs, rhs, bin_op, bin_op, bin_op);
}

multi_value cg_service::emit_dot(multi_value const &lhs,
                                 multi_value const &rhs) {
  return emit_dot_vv(lhs, rhs);
}

multi_value cg_service::emit_cross(multi_value const &lhs,
                                   multi_value const &rhs) {
...
multi_value cg_service::emit_extract_ref(multi_value const &lhs, size_t idx) {
  ...
  if (is_vector(agg_hint)) {
    ...
    for (size_t value_index = 0; value_index < value_parallel_factor;
         ++value_index) {
      elem_address[value_index] = builder().CreateStructGEP(
          agg_address[value_index]->getType(), agg_address[value_index],
    ...
    return create_value(tyinfo, elem_address, value_kinds::reference,
                        lhs.abi());
  }
  EFLIB_ASSERT_UNIMPLEMENTED();
  return multi_value();
...
multi_value cg_service::emit_extract_ref(multi_value const &lhs,
                                         multi_value const &idx) {
  assert(lhs.storable());
  ...
  if (is_vector(agg_hint)) {
    ...
    case abis::llvm: {
      ...
      for (size_t value_index = 0; value_index < parallel_factor_;
           ++value_index) {
        Type *value_ty = addr[value_index]->getType()->getPointerElementType();
        Type *element_ty = value_ty->getStructElementType(0);

        Value *first_elem_ptr = builder().CreateBitCast(
            addr[value_index], PointerType::getUnqual(element_ty));
        Value *indexes[] = {index_values[value_index]};
        elem_ptr[value_index] = builder().CreateGEP(first_elem_ptr->getType(), first_elem_ptr, indexes);
      }
      return create_value(nullptr, row_vector_of(lhs.hint()), elem_ptr,
                          value_kinds::reference, lhs.abi());
    }
    default:
      assert(false);
    }
    ...
    case abis::llvm: {
      cg_type *elem_tyinfo =
          ctxt_->get_node_context(array_tyn->elem_type.get())->ty;
      value_array index_values = idx.load();
      value_array elem_addr(parallel_factor_, nullptr);
      for (size_t value_index = 0; value_index < parallel_factor_;
           ++value_index) {
        elem_addr[value_index] =
            builder().CreateGEP(addr[value_index]->getType(), addr[value_index], index_values[value_index]);
      }
      return create_value(elem_tyinfo, elem_addr, value_kinds::reference,
                          lhs.abi());
    }
    default:
      assert(false);
...
multi_value cg_service::emit_extract_val(multi_value const &lhs, size_t idx) {
  ...
  if (agg_hint == builtin_types::none) {
    ...
    case abis::llvm: {
      ...
      for (size_t value_index = 0; value_index < parallel_factor_;
           ++value_index) {
        elem_val[value_index] =
            builder().CreateExtractElement(val[value_index], vm_index);
      }
    } break;
    default:
      assert(!"Unknown ABI");
...
multi_value cg_service::emit_extract_val(multi_value const &lhs,
                                         multi_value const &idx) {
  builtin_types agg_hint = lhs.hint();
  ...
  if (agg_hint == builtin_types::none) {
    ...
    case abis::llvm: {
      ...
      cg_type *elem_tyinfo =
          ctxt_->get_node_context(array_tyn->elem_type.get())->ty;
      return create_value(elem_tyinfo, builtin_types::none, elem_addr,
                          value_kinds::reference, lhs.abi());
    }
    default:
      assert(false);
...
multi_value cg_service::emit_extract_elem_mask(multi_value const &vec,
  ...
  if (vec.storable()) {
    multi_value swz_proxy =
        create_value(nullptr, swz_hint, invalid_value_array(),
                     value_kinds::elements, vec.abi());
    ...
    if (is_scalar(vec.hint())) {
      ...
      case abis::llvm: {
        Value *mask_mono =
            ext_->get_vector<int>(ArrayRef<char>(indexes.data, idx_len));
        value_array mask(parallel_factor_, mask_mono);
        value_array v = ext_->shuffle_vector(vec_v, vec_v, mask);
        return create_value(nullptr, swz_hint, v, value_kinds::value,
                            abis::llvm);
      }
      default:
        assert(false);
...
multi_value cg_service::emit_dot_vv(multi_value const &lhs,
                                    multi_value const &rhs) {
...
multi_value cg_service::emit_mul_mv(multi_value const &lhs,
                                    multi_value const &rhs) {
...
multi_value cg_service::emit_mul_vm(multi_value const &lhs,
                                    multi_value const &rhs) {
  assert(promote_abi(lhs.abi(), rhs.abi(), abis::llvm) == abis::llvm);
  ...
  multi_value ret =
      null_value(vector_of(scalar_of(lhs.hint()), out_v), lhs.abi());
  for (size_t idx = 0; idx < out_v; ++idx) {
    ret = emit_insert_val(ret, (int)idx,
                          emit_dot_vv(lrv, emit_extract_col(rrv, idx)));
  }

  return ret;
}

multi_value cg_service::emit_mul_mm(multi_value const &lhs,
                                    multi_value const &rhs) {
  assert(promote_abi(lhs.abi(), rhs.abi(), abis::llvm) == abis::llvm);
  ...
  for (size_t irow = 0; irow < out_r; ++irow) {
    ...
    for (size_t icol = 0; icol < out_v; ++icol) {
      row_vec =
          emit_insert_val(row_vec, (int)icol, out_cells[irow * out_v + icol]);
    }
...
multi_value cg_service::emit_abs(multi_value const &arg_value) {
  ...
  value_array ret_v = ext_->call_unary_intrin(
      v[0]->getType(), v, std::bind(&cg_extension::abs_sv, ext_.get(), _1));
...
multi_value cg_service::emit_sqrt(multi_value const &arg_value) {
  ...
  if (scalar_hint == builtin_types::_float) {
    ...
    return create_value(arg_value.ty(), arg_value.hint(), ret_v,
                        value_kinds::value, arg_abi);
  } else {
    EFLIB_ASSERT_UNIMPLEMENTED();
    return multi_value();
  }
...
multi_value cg_service::undef_value(builtin_types bt, abis abi) {
  ...
  multi_value val =
      create_value(bt, value_array(parallel_factor_, UndefValue::get(valty)),
                   value_kinds::value, abi);
  return val;
}

multi_value cg_service::emit_call(cg_function const &fn,
                                  vector<multi_value> const &args) {
...
Argument passing rules:
   ...
   Arg Passing   |    Mono LLVM ABI    |    Mono C ABI   |  Multi-Value LLVM ABI
|   Multi-Value C ABI
-----------------|---------------------|-----------------|------------------------|-----------------------
Ret  Value small |     direct, <v>     | 1st arg, <&ret> |   direct, array<ret>
| 1st arg, &array<&ret>
-----------------|---------------------|-----------------|------------------------|-----------------------
Ret  Value big   |     direct, <v>     | 1st arg, <&ret> |   direct, array<ret>
| 1st arg, &array<&ret>
-----------------|---------------------|-----------------|------------------------|-----------------------
Out  Value small |        <&v>         |      <&v>       |       array<&v> |
&array<&v>
-----------------|---------------------|-----------------|------------------------|-----------------------
Out  Value big   |        <&v>         |      <&v>       |       array<&v> |
&array<&v>
-----------------|---------------------|-----------------|------------------------|-----------------------
In   Value small |         <v>         |       <v>       |        array<v> |
&array <v>
-----------------|---------------------|-----------------|------------------------|-----------------------
In   Value big   |         <v>         |      <&v>       |        array<v> |
&array <&v>
-----------------|---------------------|-----------------|------------------------|-----------------------
...
multi_value cg_service::emit_call(cg_function const &fn,
  ...
  // Fill values for arguments.
  {
    ...
    if (fn.return_via_arg()) {
      ret_variable =
          create_variable(fn.result_type(), fn.abi(), ".temp.result");
    ...
    if (fn.need_mask()) {
      physical_args[physical_index++] =
          exec_mask ? exec_mask : current_execution_mask();
    }

    // Push arguments to list.
    for (size_t logical_index = 0; logical_index < args.size();
      ...
      if (fn.multi_value_args()) {
        ...
        if (fn.multi_value_arg_as_ref()) {
          physical_args[physical_index] =
              restore(physical_args[physical_index]);
        }
      } else {
        physical_args[physical_index] = arg_multi_values[0];
      }
  ...
  Value *ret_value =
      builder().CreateCall(fn.fn->getFunctionType(), fn.fn, physical_args);
  ...
  abis ret_abi = arg_abi;
  return create_value(fn.result_type(), ret_value_array, value_kinds::value,
                      ret_abi);
}
...
multi_value cg_service::cast_bits(multi_value const &v, cg_type *dest_tyi) {
  ...
  unary_intrin_functor bitcast_sv =
      ext_->bind_cast_sv(dest_scalar_ty, cast_ops::bitcast);
...
bool cg_service::merge_swizzle(multi_value const *&root, elem_indexes &indexes,
  ...
  while (!root->indexes().empty() &&
         root->parent()->hint() != builtin_types::none) {
    is_swizzle = true;
...
multi_value cg_service::create_value_by_scalar(multi_value const &scalar,
                                               cg_type *tyinfo,
                                               builtin_types hint) {
...
multi_value cg_service::emit_all(multi_value const &v) {
  ...
  if (is_scalar(hint)) {
    ...
    for (size_t i = 1; i < vector_size(hint); ++i) {
      elem_v = emit_extract_val(v, i);
      ret =
          emit_and(ret, emit_cmp_ne(elem_v, null_value(scalar_hint, v.abi())));
    }
    return ret;
  } else if (is_matrix(hint)) {
    EFLIB_ASSERT_UNIMPLEMENTED();
...
multi_value cg_service::emit_cmp(multi_value const &lhs, multi_value const &rhs,
  ...
  if (is_real(scalar_hint)) {
    cmp_fn = [this, pred_float](llvm::Value *lhs, llvm::Value *rhs) {
      return builder().CreateICmp(static_cast<CmpInst::Predicate>(pred_float),
                                  lhs, rhs, "");
    };
    ...
    cmp_fn = [this, pred](llvm::Value *lhs, llvm::Value *rhs) {
      return builder().CreateICmp(static_cast<CmpInst::Predicate>(pred), lhs,
                                  rhs, "");
    };
  }

  unary_intrin_functor cast_fn =
      std::bind(&cg_extension::i1toi8_sv, ext_.get(), _1);
  value_array ret =
      ext_->call_binary_intrin(ret_ty, lhs_v, rhs_v, cmp_fn, cast_fn);

  return create_value(ret_hint, ret, value_kinds::value, promoted_abi);
}

multi_value cg_service::emit_cmp_eq(multi_value const &lhs,
                                    multi_value const &rhs) {
  return emit_cmp(lhs, rhs, CmpInst::ICMP_EQ, CmpInst::ICMP_EQ,
                  CmpInst::FCMP_OEQ);
}

multi_value cg_service::emit_cmp_lt(multi_value const &lhs,
                                    multi_value const &rhs) {
  return emit_cmp(lhs, rhs, CmpInst::ICMP_SLT, CmpInst::ICMP_ULT,
                  CmpInst::FCMP_ULT);
}

multi_value cg_service::emit_cmp_le(multi_value const &lhs,
                                    multi_value const &rhs) {
  return emit_cmp(lhs, rhs, CmpInst::ICMP_SLE, CmpInst::ICMP_ULE,
                  CmpInst::FCMP_ULE);
}

multi_value cg_service::emit_cmp_ne(multi_value const &lhs,
                                    multi_value const &rhs) {
  return emit_cmp(lhs, rhs, CmpInst::ICMP_NE, CmpInst::ICMP_NE,
                  CmpInst::FCMP_UNE);
}

multi_value cg_service::emit_cmp_ge(multi_value const &lhs,
                                    multi_value const &rhs) {
  return emit_cmp(lhs, rhs, CmpInst::ICMP_SGE, CmpInst::ICMP_UGE,
                  CmpInst::FCMP_UGE);
}

multi_value cg_service::emit_cmp_gt(multi_value const &lhs,
                                    multi_value const &rhs) {
  return emit_cmp(lhs, rhs, CmpInst::ICMP_SGT, CmpInst::ICMP_UGT,
                  CmpInst::FCMP_UGT);
}
...
multi_value cg_service::emit_bin_ps_ta_sva(multi_value const &lhs,
  ...
  abis internal_abi = promote_abi(promoted_abi, abis::llvm);
  ...
  if (is_real(scalar_hint)) {
    ret =
        ext_->call_binary_intrin(ret_ty, lhs_v, rhs_v, float_sv_fn, null_unary);
  } else if (is_integer(scalar_hint)) {
    if (is_signed(scalar_hint)) {
      ret = ext_->call_binary_intrin(ret_ty, lhs_v, rhs_v, signed_sv_fn,
                                     null_unary);
    } else {
      ret = ext_->call_binary_intrin(ret_ty, lhs_v, rhs_v, unsigned_sv_fn,
                                     null_unary);
    }
  ...
  multi_value retval =
      create_value(hint, ret, value_kinds::value, internal_abi);
...
multi_value cg_service::emit_and(multi_value const &lhs,
                                 multi_value const &rhs) {
...
multi_value cg_service::emit_or(multi_value const &lhs,
                                multi_value const &rhs) {
  assert(scalar_of(lhs.hint()) == builtin_types::_boolean);
  assert(lhs.hint() == rhs.hint());
...
multi_value cg_service::emit_tex2Dlod(multi_value const &samp,
                                      multi_value const &coord) {
  return emit_tex_lod_impl(samp, coord, externals::tex2dlod_vs,
                           externals::tex2dlod_ps);
}
...
multi_value cg_service::emit_tex2Dbias(multi_value const &samp,
                                       multi_value const &coord) {
  return emit_tex_bias_impl(samp, coord, externals::tex2dbias_ps);
}

multi_value cg_service::emit_tex2Dproj(multi_value const &samp,
                                       multi_value const &coord) {
  return emit_tex_proj_impl(samp, coord, externals::tex2dproj_ps);
}

multi_value cg_service::create_constant_int(cg_type *tyinfo, builtin_types bt,
                                            abis abi, uint64_t v) {
  ...
  Value *value_mono =
      ext_->get_int(ret_ty, APInt(bits, v, is_signed(scalar_hint)));
  value_array ret(parallel_factor_, value_mono);
  return create_value(tyinfo, bt, ret, value_kinds::value, abi);
}

multi_value
cg_service::emit_unary_ps(std::string const &scalar_external_intrin_name,
                          multi_value const &v) {
  ...
  value_array ret_v = ext_->call_unary_intrin(nullptr, v.load(), intrin_sv);
  return create_value(v.ty(), v.hint(), ret_v, value_kinds::value, v.abi());
}

multi_value
cg_service::emit_bin_ps_ta_sva(std::string const &scalar_external_intrin_name,
  ...
  value_array ret_v = ext_->call_binary_intrin(
      nullptr, v0.load(abi), v1.load(abi), intrin_sv, null_unary);

  return create_value(v0.ty(), v0.hint(), ret_v, value_kinds::value, abi);
}

multi_value cg_service::extend_to_vm(multi_value const &v,
                                     builtin_types complex_hint) {
  builtin_types hint = v.hint();
  assert(is_scalar(hint));
  EFLIB_UNREF_DECLARATOR(hint);
...
multi_value
cg_service::emit_bin_es_ta_sva(std::string const &scalar_external_intrin_name,
...
multi_value cg_service::emit_tex_lod_impl(multi_value const &samp,
  ...
  value_array coord_ptr =
      ext_->stack_alloc(coord_ty, parallel_factor_, "coord.tmp");
  ...
  return create_value(nullptr, v4f32_hint, ret_ptr, value_kinds::reference,
                      abi);
}
...
multi_value cg_service::emit_tex_grad_impl(multi_value const &samp,
  ...
  value_array ddx_ptr =
      ext_->stack_alloc(coord_ty, parallel_factor_, "ddx.tmp");
  ext_->store(ddx.load(abi), ddx_ptr);

  value_array ddy_ptr =
      ext_->stack_alloc(coord_ty, parallel_factor_, "ddy.tmp");
  ext_->store(ddy.load(abi), ddy_ptr);

  value_array masks = split_mask(current_execution_mask());
  value_array args[] = {ret_ptr,   masks,   samp.load(),
                        coord_ptr, ddx_ptr, ddy_ptr};
...
multi_value cg_service::emit_tex_proj_impl(multi_value const &samp,
  ...
  value_array coord_ptr =
      ext_->stack_alloc(v4f32_ty, parallel_factor_, "coord.tmp");
  ext_->store(coord.load(abi), coord_ptr);

  value_array ddx_ptr =
      ext_->stack_alloc(v4f32_ty, parallel_factor_, "ddx.tmp");
  ext_->store(ddx.load(abi), ddx_ptr);

  value_array ddy_ptr =
      ext_->stack_alloc(v4f32_ty, parallel_factor_, "ddy.tmp");
...
multi_value cg_service::emit_texCUBElod(multi_value const &samp,
                                        multi_value const &coord) {
  return emit_tex_lod_impl(samp, coord, externals::texCUBElod_vs,
                           externals::texCUBElod_ps);
}
...
multi_value cg_service::emit_texCUBEbias(multi_value const &samp,
                                         multi_value const &coord) {
  return emit_tex_bias_impl(samp, coord, externals::texCUBEbias_ps);
}

multi_value cg_service::emit_texCUBEproj(multi_value const &samp,
                                         multi_value const &coord) {
...
multi_value cg_service::emit_select(multi_value const &flag,
  ...
  return create_value(v0.ty(), v0.hint(), ext_->select(flag_v, v0_v, v1_v),
                      value_kinds::value, promoted_abi);
}
...
multi_value cg_service::inf_from_value(multi_value const &v, bool negative) {
  ...
  Value *inf_value =
      ext_->get_constant_by_scalar(v_v[0]->getType(), scalar_inf);
  return create_value(v.ty(), v.hint(),
...
multi_value cg_service::emit_isinf(multi_value const &v) {
  ...
  return emit_cmp(abs_v, inf_from_value(v, false), ICmpInst::ICMP_EQ,
                  ICmpInst::ICMP_EQ, FCmpInst::FCMP_OEQ);
}

multi_value cg_service::emit_isfinite(multi_value const &v) {
  multi_value is_eq =
      emit_cmp(v, v, ICmpInst::ICMP_EQ, ICmpInst::ICMP_EQ, FCmpInst::FCMP_OEQ);
...
multi_value cg_service::emit_isnan(multi_value const &v) {
  return emit_cmp(v, v, ICmpInst::ICMP_EQ, ICmpInst::ICMP_EQ,
                  FCmpInst::FCMP_UNO);
}
...
multi_value cg_service::emit_clamp(multi_value const &v,
                                   multi_value const &min_v,
...
multi_value cg_service::numeric_value(multi_value const &proto, double fp,
                                      uint64_t ui) {
  Type *ty = nullptr;
  if (proto.ty()) {
  ...
  return create_value(proto.ty(), proto.hint(), ret_value, value_kinds::value,
                      proto.abi());
}
...
multi_value cg_service::create_scalar(Value *val, cg_type *tyinfo,
                                      builtin_types hint) {
  assert(is_scalar(hint));
  return create_value(tyinfo, hint, value_array(parallel_factor_, val),
                      value_kinds::value, abis::llvm);
}
...
Value *cg_service::combine_flags(value_array const &flags) {
  ...
  for (size_t i_flag = 0; i_flag < parallel_factor_; ++i_flag) {
    Value *flag_u32 =
        builder().CreateZExt(flags[i_flag], Type::getInt32Ty(context()));
