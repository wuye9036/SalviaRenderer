...
bool parse_results::is_continuable() const {
  return (tag & parse_result_values::recover_failed_mask) != parse_result_values::failed;
}
bool parse_results::is_expected_failed_or_recovered() const {
...
expectation_failure::expectation_failure(token_iterator iter, parser const *p) : iter(iter), p(p) {
  ...
  if (r) {
    ...
    what_str = str(boost::format("can't match unknown parser at 0x%016p") % p);
#else
    what_str = str(boost::format("can't match unknown parser at 0x%08p") % p);
#endif
  }
}
    ...
    if (term) {
      ...
      if (rule_wrp) {
        ...
        if (inner_term) {
          ...
          shared_ptr<error_catcher> err_catcher(
              new error_catcher(p, get_expected_failed_handler(rule_wrp->get_rule()->name())));
          exprlst.push_back(err_catcher);
          return *this;
        }
      }
...
parse_results negnativer::parse(token_iterator &iter, token_iterator end,
  ...
  return (expr->parse(iter, end, attr, diags).is_succeed()) ? parse_results::failed
...
#if OUTPUT_GRAMMAR_MATCHING_PATH
  current_path->attributes.push_back(
      make_pair("diag_count", boost::format("\"%d\"") % chat->items().size()));
  if (result.is_succeed()) {
    ...
    current_path->attributes.push_back(
        make_pair("state", boost::format("\"%s\"") % "expected_failed"));
  } else if (result.is_recovered_expected_failed()) {
    current_path->attributes.push_back(
        make_pair("state", boost::format("\"%s\"") % "recovered_expected_failed"));
  }

  current_path->attributes.push_back(make_pair(
      "position", boost::format("\"%d, %d\"") % (*iter)->span.line_beg % (*iter)->span.col_beg));
  current_path->attributes.push_back(
      make_pair("token_content", boost::format("\"%s\"") % (*iter)->str));
