...
vector<shared_ptr<declaration>> syntax_tree_builder::build_decl(shared_ptr<attribute> attr) {
  ...
  EFLIB_ASSERT_AND_IF(typed_attr->selected_idx >= 0,
                      "Attribute error: least one branch was selected.") {
    return ret;
  }
...
vector<shared_ptr<declaration>> syntax_tree_builder::build_basic_decl(shared_ptr<attribute> attr) {
  ...
  EFLIB_ASSERT_AND_IF(not_empty_typed_attr->attrs.size() == 2,
                      "Basic declaration must not a empty queuer.") {
    return ret;
  }
  ...
  SASL_CASE_RULE(vardecl) {
    vector<shared_ptr<variable_declaration>> decls = build_vardecl(typed_decl_attr->attr);
...
syntax_tree_builder::build_vardecl(shared_ptr<attribute> attr) {
...
shared_ptr<function_full_def> syntax_tree_builder::build_fndecl(shared_ptr<attribute> attr) {
  shared_ptr<function_full_def> ret =
      create_node<function_full_def>(attr->token_beg(), attr->token_end());
  ...
  if (!optional_params_attr->attrs.empty()) {
    ...
    for (shared_ptr<attribute> comma_param_attr : follow_params_attr->attrs) {
      ret->params.push_back(build_param(comma_param_attr->child(1)));
    }
  }
...
shared_ptr<parameter_full> syntax_tree_builder::build_param(shared_ptr<attribute> attr) {
  shared_ptr<parameter_full> ret =
      create_node<parameter_full>(attr->token_beg(), attr->token_end());
...
void syntax_tree_builder::build_struct_body(shared_ptr<attribute> attr,
                                            shared_ptr<struct_type> out) {
  assert(out);
  SASL_DYNCAST_ATTRIBUTE(queuer_attribute, typed_attr, attr);
...
shared_ptr<expression_list> syntax_tree_builder::build_exprlst(shared_ptr<attribute> attr) {
  shared_ptr<expression_list> ret =
      create_node<expression_list>(attr->token_beg(), attr->token_end());
...
shared_ptr<expression> syntax_tree_builder::build_callexpr(shared_ptr<attribute> attr,
                                                           shared_ptr<expression> expr) {
  shared_ptr<call_expression> ret =
      create_node<call_expression>(expr->token_begin(), attr->token_end());
  ret->expr = expr;
...
shared_ptr<expression> syntax_tree_builder::build_indexexpr(shared_ptr<attribute> attr,
                                                            shared_ptr<expression> expr) {
  shared_ptr<index_expression> ret =
      create_node<index_expression>(expr->token_begin(), attr->token_end());
  ret->expr = expr;
  ret->index_expr = build_expr(attr);
  return ret;
}

shared_ptr<expression> syntax_tree_builder::build_memexpr(shared_ptr<attribute> attr,
                                                          shared_ptr<expression> expr) {
  shared_ptr<member_expression> ret =
      create_node<member_expression>(expr->token_begin(), attr->token_end());
  ret->expr = expr;
...
shared_ptr<expression> syntax_tree_builder::build_pmexpr(shared_ptr<attribute> attr) {
  ...
  SASL_CASE_RULE(lit_const) {
    ...
    shared_ptr<constant_expression> ret =
  ...
  SASL_CASE_RULE(ident) {
    ...
    shared_ptr<variable_expression> varexpr =
        create_node<variable_expression>(var_attr->tok, var_attr->tok);
...
shared_ptr<cast_expression> syntax_tree_builder::build_typecastedexpr(shared_ptr<attribute> attr) {
  ...
  shared_ptr<cast_expression> ret =
      create_node<cast_expression>(attr->token_beg(), attr->token_end());
  ret->casted_type = ty;
  ret->expr = expr;
...
shared_ptr<compound_statement>
syntax_tree_builder::build_stmt_compound(shared_ptr<attribute> attr) {
...
shared_ptr<jump_statement> syntax_tree_builder::build_flowctrl(shared_ptr<attribute> attr) {
  shared_ptr<jump_statement> ret =
      create_node<jump_statement>(attr->token_beg(), attr->token_end());
...
shared_ptr<expression_statement> syntax_tree_builder::build_stmt_expr(shared_ptr<attribute> attr) {
  shared_ptr<expression_statement> ret =
      create_node<expression_statement>(attr->token_beg(), attr->token_end());
...
shared_ptr<declaration_statement> syntax_tree_builder::build_stmt_decl(shared_ptr<attribute> attr) {
  shared_ptr<declaration_statement> ret =
      create_node<declaration_statement>(attr->token_beg(), attr->token_end());
...
shared_ptr<while_statement> syntax_tree_builder::build_stmt_while(shared_ptr<attribute> attr) {
  ...
  if (cond && stmt) {
    shared_ptr<while_statement> ret =
        create_node<while_statement>(attr->token_beg(), attr->token_end());
    ret->cond = cond;
    ret->body = stmt;
    return ret;
  }
...
shared_ptr<dowhile_statement> syntax_tree_builder::build_stmt_dowhile(shared_ptr<attribute> attr) {
  ...
  if (cond && stmt) {
    shared_ptr<dowhile_statement> ret =
        create_node<dowhile_statement>(attr->token_beg(), attr->token_end());
    ret->cond = cond;
    ret->body = stmt;
    return ret;
  }
...
shared_ptr<switch_statement> syntax_tree_builder::build_stmt_switch(shared_ptr<attribute> attr) {
  ...
  shared_ptr<switch_statement> ret =
      create_node<switch_statement>(attr->token_beg(), attr->token_end());
  ret->cond = cond;
  ret->stmts = stmts;
  return ret;
}
...
shared_ptr<compound_statement> syntax_tree_builder::wrap_to_compound(shared_ptr<statement> stmt) {
  ...
  shared_ptr<compound_statement> ret_stmt =
      create_node<compound_statement>(stmt->token_begin(), stmt->token_end());
...
operators syntax_tree_builder::build_binop(shared_ptr<attribute> attr, token &op_tok) {
  ...
  case '|':
    if (op_chars[1] == '|')
      return operators::logic_or;
    if (op_chars[1] == '=')
      return operators::bit_or_assign;
    if (op_chars[1] == '\0')
      return operators::bit_or;
    break;
  case '&':
    if (op_chars[1] == '&')
      return operators::logic_and;
    if (op_chars[1] == '=')
      return operators::bit_and_assign;
    if (op_chars[1] == '\0')
      return operators::bit_and;
    break;
  case '^':
    if (op_chars[1] == '=')
      return operators::bit_xor_assign;
    if (op_chars[1] == '\0')
      return operators::bit_xor;
    break;
  }
