#include <sasl/include/syntax_tree/operator_literal.h>

sasl::syntax_tree::operator_table::operator_table()
{
	add( "+", operators::add );
...
operator_table& sasl::syntax_tree::operator_table::add( const std::string& lit, operators op )
{
	lit2op.insert( std::make_pair( lit, op ) );
	op2lit.insert( std::make_pair( op, lit ) );
}

operator_table& sasl::syntax_tree::operator_table::instance()
{
	static operator_table op_tbl;
}

operators sasl::syntax_tree::operator_table::find( const string& lit, bool is_unary /*= false*/, bool is_postfix /*= false*/ ) const
{
	return
		lit2op[lit] 
...
const string& sasl::syntax_tree::operator_table::find( operators op ) const
{
	return op2lit[op];
}