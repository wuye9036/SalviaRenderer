...
void init_lex(lexer &l) {
  ...
  l.define_tokens(
      {{"lit_int", "({DIGIT}+{INT_TYPE_SUFFIX}?)|(0x{HEX_DIGIT}+{INT_TYPE_SUFFIX}?)"},
       {"lit_float",
        "({DIGIT_SEQ}?{DOT}{DIGIT_SEQ}{EXPONENT_PART}?{REAL_TYPE_SUFFIX}?)|({DIGIT_SEQ}{"
       ...
       {"space", "{SPACE}"},
       {"newline", "{NEWLINE}"},
       {"single_line_comment", "{SLASH}{SLASH}[^\\n]*"},
       {"block_comment_beg", "{SLASH}{ASTERISK}"},
  ...
  l.enable_tokens(
      "SKIPPED",
      {{"space"}, {"newline"}, {"single_line_comment"}, {"block_comment_begin", "COMMENT"}});
  l.enable_tokens("COMMENT", {{"block_comment_end", "INITIAL"}, {"any_char"}});

  l.set_skippers({"SKIPPED", "COMMENT"});
