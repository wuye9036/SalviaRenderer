...
template <typename Type> struct is_matrix_type : std::false_type {};
...
template <typename T> inline constexpr bool is_matrix_v = is_matrix_type<T>::value;

template <typename ScalarT, int ColumnSize, int RowSize, int ColumnStride = 4> struct matrix_data;

template <typename ScalarT, int Size> struct vector_;

template <typename ScalarT, int ColumnSize, int RowSize> struct det_ {};
...
template <typename ScalarT, int ColumnSize, int RowSize, int ColumnStride> struct matrix_data {
  ...
  template <typename IndexT> vector_<ScalarT, RowSize> get_column(IndexT i) const {
  ...
  template <typename IndexT> void set_column(IndexT i, vector_<ScalarT, RowSize> const &v) {
    for (int i_row = 0; i_row < RowSize; ++i_row) {
      data_[i_row][i] = v[i_row];
    }
  }

  template <typename IndexT> vector_<ScalarT, ColumnSize> const &get_row(IndexT i) const {
    return *reinterpret_cast<vector_<ScalarT, ColumnSize> const *>(&(data_[i][0]));
  }

  template <typename IndexT> void set_row(IndexT i, vector_<ScalarT, ColumnSize> const &v) {
...
struct matrix_operators<ScalarT, ColumnSize, RowSize, ColumnStride, true>
  ...
  template <typename ScalarIt> static matrix_type diag(ScalarIt begin, ScalarIt end) {
