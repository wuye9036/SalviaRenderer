...
namespace eflib {
template <class T> T sign(const T &in) { return in >= T(0) ? T(1) : T(-1); }

template <class T> bool equal(const T &in1, const T &in2) {
  if ((T)(std::abs(in1 - in2)) <= std::numeric_limits<T>::epsilon())
    return true;
  return false;
}
...
template <class T, class U> T round(U d) {
  static_assert(std::is_floating_point<U>::value);
  static_assert(std::is_integral<T>::value);
  return (T)(d + 0.5);
}

template <typename T, typename U> T round_up(T v, U align) {
...
template <class T> T trunc(T d) {
  static_assert(std::is_floating_point<T>::value);
  return d > static_cast<T>(0) ? fast_floor(d) : fast_ceil(d);
}

template <class T, class U> void round(T &t, U d) {
  static_assert(std::is_floating_point<U>::value);
  static_assert(std::is_integral<T>::value);
  t = (T)floor(d + 0.5);
}
...
template <class T> T radians(T angle) {
  static_assert(std::is_floating_point<T>::value);
...
inline int fast_ceili(double d) {
  const double DOUBLE_MAGIC_ROUND_EPS = (0.5f - 1.5e-8); // almost .5f = .5f - 1e^(number of exp
                                                         // bit)
...
inline int fast_ftol(float f) { return _mm_cvtt_ss2si(_mm_load_ss(&f)); }
#else
...
template <class Vec> Vec lerp(const Vec &v1, const Vec &v2, float t) {
...
inline __m128 &to_m128(vec4 &v) { return reinterpret_cast<__m128 &>(v); }

inline __m128 const &to_m128(vec4 const &v) { return reinterpret_cast<__m128 const &>(v); }

inline vec4 &to_vec4(__m128 &v) { return reinterpret_cast<vec4 &>(v); }

inline vec4 const &to_vec4(__m128 const &v) { return reinterpret_cast<vec4 const &>(v); }
