 ...
 * operator () is called by the pool to execute the task. Exceptions are ignored. \param Scheduler A
 * task container which determines how tasks are scheduled. It is guaranteed that this container is
 * accessed only by one thread at a time. The scheduler shall not throw exceptions.
 *
...
public:
  ...
  bool wait(std::chrono::duration<Rep, Period> const& timestamp,
    ...
    if (0 == task_threshold) {
      while (0 != self->m_active_worker_count || !self->m_scheduler.empty()) {
        if (!self->m_worker_idle_or_terminated_event.wait_for(lock, timestamp))
          return false;
      }
    } else {
      while (task_threshold < self->m_active_worker_count + self->m_scheduler.size()) {
