 ...
 * that is, the behavior of concurrent calls is as if the calls have been issued sequentially in an
 * unspecified order.
 *
 * \param Task A function object which implements the operator 'void operator() (void) const'. The
 * operator () is called by the pool to execute the task. Exceptions are ignored. \param
 * SchedulingPolicy A task container which determines how tasks are scheduled. It is guaranteed that
 * this container is accessed only by one thread at a time. The scheduler shall not throw
 * exceptions.
 *
...
public:
   ...
   * The pool's actual number threads depends on the SizePolicy.
   */
   ...
   * \see SizePolicy
   */
  size_controller_type size_controller() { return m_core->size_controller(); }
   ...
   */
  bool schedule(task_type const& task) { return m_core->schedule(task); }

  /*! Returns the number of tasks which are currently executed.
   * \return The number of active tasks.
   */
  size_t active() const { return m_core->active(); }

  /*! Returns the number of tasks which are ready for execution.
   * \return The number of pending tasks.
   */
  size_t pending() const { return m_core->pending(); }
   ...
   */
  void wait(size_t task_threshold = 0) const { m_core->wait(task_threshold); }
 ...
 *
 */
typedef thread_pool<task_func, fifo_scheduler, static_size, resize_controller, wait_for_all_tasks>
    fifo_pool;
 ...
 *
 */
typedef thread_pool<task_func, lifo_scheduler, static_size, resize_controller, wait_for_all_tasks>
    lifo_pool;
