...
inline void *aligned_malloc(size_t size, uint32_t alignment) {
  ...
  uint8_t *new_p = reinterpret_cast<uint8_t *>((reinterpret_cast<size_t>(p) + 2 + (alignment - 1)) &
...
public:
  ...
  pointer allocate(size_type count) {
    ...
    uint8_t *new_p = reinterpret_cast<uint8_t *>(
        (reinterpret_cast<size_t>(p) + 2 + (alignment - 1)) & (-static_cast<int32_t>(alignment)));
    reinterpret_cast<uint16_t *>(new_p)[-1] = static_cast<uint16_t>(new_p - p);
...
inline bool operator==(const aligned_allocator<T, alignment1> &,
                       const aligned_allocator<U, alignment2> &) throw() {
  return true;
}
...
template <int alignment> class aligned_allocator<void, alignment> {
public:
...
template <int aligned_size> struct is_aligned {
  template <class T> bool check(T p) {
...
template <typename T> class unified_allocator {
public:
  typedef T value_type;
  typedef value_type *pointer;
  typedef value_type &reference;
  ...
  template <typename U> unified_allocator(const unified_allocator<U> &) throw() {}

  ~unified_allocator() throw() {}

  template <typename U> unified_allocator<T> &operator=(const unified_allocator<U> &) {
    return *this;
  }
...
template <> class unified_allocator<void> {
public:
  typedef void *pointer;
  typedef const void *const_pointer;
  typedef void value_type;
  ...
  template <typename U, int> unified_allocator(const unified_allocator<U> &) throw() {}

  template <typename U> unified_allocator<void> &operator=(const unified_allocator<U> &) {
    return *this;
  }
};
} // namespace eflib
