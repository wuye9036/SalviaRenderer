...
struct voidptr {
  ...
  template <class T>
  friend T* voidptr_cast(const voidptr& ptr);

  voidptr() : pval(nullptr), id(type_encode<empty>::id) {}

  template <class T>
  voidptr(T* rhs) : pval(static_cast<void*>(rhs))
                  , id(type_encode<T>::id) {}
...
struct const_voidptr {
  ...
  template <class T>
  friend const T* voidptr_cast(const const_voidptr& ptr);

  const_voidptr() : pval(nullptr), id(type_encode<empty>::id) {}

  template <class T>
  const_voidptr(const T* rhs) : pval(static_cast<const void*>(rhs))
                              , id(type_encode<T>::id) {}
