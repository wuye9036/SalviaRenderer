...
template <class SerializerT> class log_state {
public:
...
// Slog state stack
template <class SerializerT> class log_state_stack {
public:
...
/// slog is not only the short name of "soft art log" but also the shorten for
/// "simple log" class slog API of our log system, serialization is supported by
/// log_serializer.
template <class SerializerT> class log {
...
public:
  ...
  static std::shared_ptr<this_type> create(const uint32_t ref_log_level,
                                           uint32_t /*log_open_mode*/) {
  ...
  template <class T> bool write(const T &val, const uint32_t cur_log_level) {
    if (!loglvlcmp_(cur_log_level, ref_log_level_)) {
      return false;
    }
    if (!logserializer_ptr) {
...
template <class T> class log_state_scope {
public:
  log_state_scope(T *plog) : plog_(plog) { plog->push_current_log_state(); }

  ~log_state_scope() { plog_->pop_current_log_state(); }
...
template <class T> class log_serializer_indent_scope {
public:
  log_serializer_indent_scope(T *pserializer) : pserializer_(pserializer) {
    pserializer->begin_log();
  }

  ~log_serializer_indent_scope() { pserializer_->end_log(); }
...
template <class LogT> class log_system {
public:
  typedef LogT log_t;
